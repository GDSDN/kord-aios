...
import * as kordJunior from "../agents/dev-junior"
...
afterEach(() => {
  ...
  ;(kordJunior.createDevJuniorAgentWithOverrides as any)?.mockRestore?.()
...
describe("Dev-Junior model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    ...
    expect(agentConfig["dev-junior"]?.model).toBe(
      kordJunior.DEV_JUNIOR_DEFAULTS.model
    )
  })

  test("uses explicitly configured dev-junior model", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      agents: {
        "dev-junior": {
...
describe("Plan agent behavior", () => {
  test("orders core agents as kord -> dev -> planner", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      kord_agent: {
    ...
    // #then - check that kord and planner are present and kord comes before planner
    const keys = Object.keys(config.agent as Record<string, unknown>)
    const kordIndex = keys.indexOf("kord")
    const plannerIndex = keys.indexOf("planner")
    expect(kordIndex).toBeGreaterThanOrEqual(0)
    expect(plannerIndex).toBeGreaterThanOrEqual(0)
    expect(kordIndex).toBeLessThan(plannerIndex)
  })

  test("plan agent should be demoted to subagent without inheriting plan prompt", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      kord_agent: {
        planner_enabled: true,
    ...
    // #then - OpenCode native plan is demoted to subagent, Kord planner is registered
  ...
  test("plan agent remains unchanged when planner is disabled", async () => {
    ...
    // #then - OpenCode native plan is not touched, Kord planner is not created
    const agents = config.agent as Record<string, { mode?: string; name?: string; prompt?: string }>
...
describe("Agent permission defaults", () => {
  test("dev should allow task", async () => {
    // #given
...
describe("Plan direct override priority over category", () => {
  ...
  test("planner prompt_append is appended to base prompt", async () => {
    ...
    expect(agents.planner).toBeDefined()
    expect(agents.planner.prompt).toContain("Plan")
