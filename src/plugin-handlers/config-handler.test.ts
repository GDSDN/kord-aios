...
import * as devAgent from "../agents/dev";
...
afterEach(() => {
  ...
  (devAgent.createDevAgentWithOverrides as any)?.mockRestore?.();
...
describe("Dev model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    ...
    expect(agentConfig.dev?.model).toBe(devAgent.DEV_DEFAULTS.model);
  });

  test("uses explicitly configured legacy sisyphus-junior model", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      agents: {
        "sisyphus-junior": {
    ...
    // #then
    const agentConfig = config.agent as Record<string, { model?: string }>;
    expect(agentConfig.dev?.model).toBe("openai/gpt-5.3-codex");
  });

  test("uses canonical dev model when configured", async () => {
    // #given
    const pluginConfig: OhMyOpenCodeConfig = {
      agents: {
        dev: {
    ...
    // #then
    const agentConfig = config.agent as Record<string, { model?: string }>;
    expect(agentConfig.dev?.model).toBe("openai/gpt-5.2");
  });
});
