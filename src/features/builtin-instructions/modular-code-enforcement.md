...
**Self-test**: If you cannot describe the file's purpose in ONE short phrase (for example, `parses YAML frontmatter` or `matches rules against file paths`), the file does too much. Split it.
...
| File has both types and implementation | SPLIT NOW -- `types.ts` + implementation |
| You need to scroll to understand the file | SPLIT NOW -- it is too large |
...
1. Check the file you are touching -- does it violate any rule above?
...
4. If adding code to an existing file -- verify the addition does not push the file past 200 LOC or add a second responsibility. If it does, extract into a new module.
