---
name: setup-github
description: "setup-github methodology and workflow"
agent: devops
subtask: false
---
...
Configure complete GitHub DevOps infrastructure for user projects created with Kord AIOS. This task copies GitHub Actions workflows, configures CodeRabbit, sets up branch protection, and manages secrets.
...
acceptance-criteria:
  ...
  - [ ] Setup report documents all configurations
    ...
    blocker: true
    validation: |
      .kord-aios/devops-setup-report.yaml contains all setup details
...
# Check idempotency
if (Test-Path ".kord-aios/devops-setup-report.yaml") {
...
$ciTemplate = Get-Content "ci.yml.template" -Raw
...
$coderabbitConfig = Get-Content "coderabbit.yaml.template" -Raw
...
# Ensure .kord-aios directory exists
New-Item -ItemType Directory -Path ".kord-aios" -Force | Out-Null

$report | Out-File -FilePath ".kord-aios/devops-setup-report.yaml" -Encoding utf8

Write-Host "✅ Setup report saved to .kord-aios/devops-setup-report.yaml"
```
...
║     git add .github/ .coderabbit.yaml .kord-aios/                              ║
║     git commit -m "chore: add DevOps configuration [Story 5.10]"          ║
║     git push                                                               ║
║                                                                            ║
║  Report saved: .kord-aios/devops-setup-report.yaml                             ║
║                                                                            ║
╚═══════════════════════════════════════════════════════════════════════════╝
