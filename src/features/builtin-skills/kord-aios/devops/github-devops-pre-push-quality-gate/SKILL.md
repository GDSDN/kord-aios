...
description: "Execute comprehensive quality checks before pushing code to remote repository, ensuring code quality, tests, and security standards are met."
agent: devops
subtask: false
---
...
Execute comprehensive quality checks before pushing code to remote repository, ensuring code quality, tests, and security standards are met.
...
- Repository context detected (run `kord-aios init` if needed)
...
if (!context) {
  ...
  console.error('Run "kord-aios init" to configure installation mode');
  process.exit(1);
}
