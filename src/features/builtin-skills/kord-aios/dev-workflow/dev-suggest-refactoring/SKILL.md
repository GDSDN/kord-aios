...
description: "Suggest Refactoring - Kord AIOS Developer Task methodology and workflow"
agent: dev
subtask: false
---

# Suggest Refactoring - Kord AIOS Developer Task
...
```bash
# Analyze single file
suggest-refactoring kord-aios-core/scripts/complex-utility.js

# Analyze directory with specific patterns
suggest-refactoring kord-aios-core/agents --patterns extract_method,decompose_class --recursive

# Apply high-impact suggestions
suggest-refactoring kord-aios-core/utils --threshold 7 --apply-all

# Export suggestions for review
suggest-refactoring . --recursive --export refactoring-report.json

# Dry run to see changes
suggest-refactoring kord-aios-core/agents/developer.md --apply ref-001 --dry-run
```
...
class SuggestRefactoringTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('🔧 Kord AIOS Refactoring Analysis'));
  ...
  async loadSuggestion(suggestionId) {
    ...
    const cacheFile = path.join(this.rootPath, '.kord-aios', 'refactoring', `${suggestionId}.json`);
