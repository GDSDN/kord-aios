...
description: "Cleanup Utilities Task methodology and workflow"
agent: dev
subtask: false
---
...
mkdir -p .backups
cp -r utils ".backups/utils.backup-3.18-$(date +%Y%m%d-%H%M%S)"
```
...
grep -r "require.utility-name" agents tasks workflows Squads/

# Check for string references (utility mentioned in docs/configs)
grep -r "utility-name"   core-config.yaml

# Count total references
count=$(grep -r "utility-name" .kord-aios-core/ Squads/ 2>/dev/null | wc -l)
...
```bash
rm -rf utils
cp -r .backups/utils.backup-3.18-YYYYMMDD-HHMMSS utils
```

*Selective Restoration**:
```bash
cp specific-utility.js 
```
...
git mv utility-name.js 

# Or for multiple files:
git mv {kord-aios-validator-fixed.js,kord-aios-validator-refactored.js} 
```
...
git mv kord-aios-validator.test.js tests/utils/
```
...
1. kord-aios-validator-fixed.js - Duplicate of kord-aios-validator.js
2. kord-aios-validator-refactored.js - Duplicate of kord-aios-validator.js
...
...
Update utility count in `core-config.yaml`:
...
for util in $(ls *.js); do
  name=$(basename $util .js)
  refs=$(grep -r "require.*$name" agents tasks 2>/dev/null | wc -l)
  if [ $refs -gt 0 ]; then
...
- ✅ Framework validation (kord-aios-validator.js) passes
- ✅ All agents (@dev, @po, @qa) activate successfully
