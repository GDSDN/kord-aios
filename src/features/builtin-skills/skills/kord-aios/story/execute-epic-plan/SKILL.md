...
4. *Error:** State file corrupted
   ...
   - *Recovery:** Restore from .kord-aios/epic-{epicId}-state.yaml.bak
...
- Template `epic-orchestration.yaml` exists in ``
- Inner loop `development-cycle.yaml` exists in ``
...
```yaml
# .kord-aios/epic-{epicId}-state.yaml
epic_state:
  epicId: {epicId}
...
**1. Load state** from `.kord-aios/epic-{epicId}-state.yaml`
...
State preserved at: .kord-aios/epic-{epicId}-state.yaml
...
PROCEDURE execute_wave(wave, stories, state):
  ...
  Task tool call:
    ...
    subagent_type: "kord-aios-architect"  # or whatever the gate agent is
    prompt: |
  ...
  IF gate verdict == APPROVED:
    ...
    Task tool call:
      ...
      subagent_type: "kord-aios-devops"
      prompt: |
...
PROCEDURE final_gate(execution, state):
  ...
  Task tool call:
    ...
    subagent_type: "kord-aios-architect"
    prompt: |
