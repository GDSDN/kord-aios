...
class ReviewProposalTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📋 Kord AIOS Proposal Review'));
  ...
  async loadProposal(proposalId) {
    try {
      const proposalFile = path.join(this.rootPath, '.kord-aios', 'proposals', `${proposalId}.json`);
  ...
  async checkForConflicts(proposal) {
    ...
    const indexFile = path.join(this.rootPath, '.kord-aios', 'proposals', 'index.json');
  ...
  async getReviewDetails(proposal, analysis, config) {
    ...
    // Set reviewer info
    details.reviewer = process.env.USER || 'kord-aios-reviewer';
  ...
  async storeReview(proposal, review) {
    const reviewsDir = path.join(this.rootPath, '.kord-aios', 'proposals', 'reviews');
  ...
  async updateProposalStatus(proposal, review) {
    ...
    const proposalFile = path.join(this.rootPath, '.kord-aios', 'proposals', `${proposal.proposalId}.json`);
