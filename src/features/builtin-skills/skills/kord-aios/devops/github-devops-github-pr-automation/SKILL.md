---
name: github-devops-github-pr-automation
description: "Automate PR creation from story context using GitHub CLI, works with ANY repository."
agent: devops
subtask: false
---
...
Automate PR creation from story context using GitHub CLI, works with ANY repository.
...
if (!context) {
  throw new Error('Unable to detect repository. Run "kord-aios init" first.');
}
```
...
function loadPRConfig() {
  const configPath = path.join(process.cwd(), '.kord-aios-core', 'core-config.yaml');
...
### NPM Package with Semantic-Release (Kord AIOS-core)
