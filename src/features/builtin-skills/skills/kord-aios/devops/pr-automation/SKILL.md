---
name: pr-automation
description: "Automate Pull Request Creation for Open-Source Contributions methodology and workflow"
agent: devops
subtask: false
---
...
To help users contribute to the Kord AIOS open-source project (`@kord-aios/kord-aios-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

*Target Repository**: `@kord-aios/kord-aios-core` (open-source framework)
...
- *contribution_path**: `string`
  ...
  - *Example**: `"Squads/my-new-pack/"` or `"kord-aios-core/agents/improved-agent.md"`
...
- *pr_url**: `string`
  ...
  - *Example**: `"https://github.com/GDSDN/kord-aios/pull/123"`
...
2. *Validate Repository State**
   - Check if `@kord-aios/kord-aios-core` repository is set as upstream
...
3. *Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `kord-aios-core/agents/*` → "agent"
     - `kord-aios-core/tasks/*` → "task"
     - `kord-aios-core/tools/*` → "tool"
...
14. *Generate PR Title & Description**
    ...
    - *Description**: Use PR template:
      ...
      *First-time contributor?** Welcome! 🎉 This PR was created using Kord AIOS PR Automation.
      ```
...
18. *Provide Next Steps**
    - Display to user:
      ...
      Thank you for contributing to Kord AIOS! 🚀
      ```
...
```bash
kord-aios activate Otto  # github-devops agent
kord-aios pr create \
  --type="Squad" \
...
kord-aios pr create \
  --type="agent" \
  --path="kord-aios-core/agents/improved-po.md" \
...
kord-aios pr create \
  --type="bug-fix" \
  --path="kord-aios-core/tasks/create-next-story.md" \
