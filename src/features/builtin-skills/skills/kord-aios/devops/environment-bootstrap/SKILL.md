...
description: "environment-bootstrap methodology and workflow"
agent: devops
subtask: false
---
...
Complete environment bootstrap for new Kord AIOS projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
acceptance-criteria:
  ...
  - [ ] Project structure follows Kord AIOS conventions
    type: acceptance-criterion
    blocker: false
    validation: |
      Check docs/, .kord-aios/, and package.json exist
...
ESSENTIAL (required for Kord AIOS):
...
# Kord AIOS generated files
.kord-aios/project-status.yaml
.kord-aios/environment-report.yaml
...
> Created with Kord AIOS Kord AIOS
...
---
Generated by Kord AIOS Environment Bootstrap*
...
- Initialize Kord AIOS Kord AIOS project structure
...
*Action:** Create Kord AIOS-compliant project structure
...
# Create .kord-aios/config.yaml
@"
# Kord AIOS Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".kord-aios/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with Kord AIOS Kord AIOS",
  "scripts": {
...
*Action:** Ask user for their profile preference and persist to `~/.kord-aios/user-config.yaml`

*Elicitation Point (PRD §2.4):**

```
🤖 Bem-vindo ao Kord AIOS!
...
```bash
# Create ~/.kord-aios/ with secure permissions
mkdir -p ~/.kord-aios
chmod 700 ~/.kord-aios

# Write user-config.yaml with selected profile
cat > ~/.kord-aios/user-config.yaml << EOF
# Kord AIOS User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('config-resolver');
...
- `~/.kord-aios/` directory must have permissions 700
- `~/.kord-aios/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".kord-aios/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .kord-aios/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ Kord AIOS ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
╠═══════════════════════════════════════════════════════════════════════════╣
║  my-awesome-project/                                                       ║
║  ├── .kord-aios/                    # Kord AIOS configuration                        ║
...
║                                                                            ║
║  Full workflow: greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .kord-aios/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • Kord AIOS help: @kord-aios-master help                                           ║
...
- [Kord AIOS Greenfield Workflow](greenfield-fullstack.yaml)
