...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	sections.push(``)
	sections.push(`Add Kord attribution to EVERY commit:`)
	sections.push(``)

	if (commitFooter) {
		sections.push(`1. **Footer in commit body:**`)
		sections.push("```")
		sections.push(`Ultraworked with [Kord](https://github.com/GDSDN/kord-aios)`)
		sections.push("```")
		sections.push(``)
	}
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push(`Co-authored-by: Kord <kord-aios@users.noreply.github.com>`)
		sections.push("```")
		sections.push(``)
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Kord](https://github.com/GDSDN/kord-aios)" -m "Co-authored-by: Kord <kord-aios@users.noreply.github.com>"`)
		sections.push("```")
	} else if (commitFooter) {
		sections.push(`**Example:**`)
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Ultraworked with [Kord](https://github.com/GDSDN/kord-aios)"`)
		sections.push("```")
	} else if (includeCoAuthoredBy) {
		sections.push(`**Example:**`)
		sections.push("```bash")
		sections.push(`git commit -m "{Commit Message}" -m "Co-authored-by: Kord <kord-aios@users.noreply.github.com>"`)
		sections.push("```")
	}
