...
describe("migrateAgentNames", () => {
  test("migrates legacy OmO names to lowercase", () => {
    ...
    expect(migrated["kord"]).toEqual({ temperature: 0.5 })
  ...
  test("handles case-insensitive migration", () => {
    ...
    const agents = {
      KORD: { model: "test" },
      "planner-kord": { prompt: "test" },
      "Orchestrator-Kord": { model: "openai/gpt-5.2" },
    }
    ...
    expect(migrated["kord"]).toEqual({ model: "test" })
    expect(migrated["planner"]).toEqual({ prompt: "test" })
    expect(migrated["builder"]).toEqual({ model: "openai/gpt-5.2" })
  })
  ...
  test("migrates orchestrator-kord to builder", () => {
    // given: Config with legacy orchestrator-kord agent name
    const agents = {
      "orchestrator-kord": { model: "anthropic/claude-opus-4-6" },
    }

    // when: Migrate agent names
    const { migrated, changed } = migrateAgentNames(agents)

    // then: orchestrator-kord should be migrated to builder
    expect(changed).toBe(true)
    expect(migrated["builder"]).toEqual({ model: "anthropic/claude-opus-4-6" })
    expect(migrated["orchestrator-kord"]).toBeUndefined()
  })

  test("migrates lowercase builder to builder", () => {
    // given: Config with lowercase builder agent name
    const agents = {
    ...
    // then: lowercase builder should remain builder (no change needed)
    expect(changed).toBe(false)
    expect(migrated["builder"]).toEqual({ model: "anthropic/claude-opus-4-6" })
  })

  test("migrates Kord variants to lowercase", () => {
    // given agents config with "Kord" key
    // when migrateAgentNames called
    // then key becomes "kord"
    const agents = { "Kord": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["kord"]).toEqual({ model: "test" })
    expect(migrated["Kord"]).toBeUndefined()
  })

  test("migrates omo key to kord", () => {
  ...
  test("migrates Atlas variants to lowercase", () => {
    ...
    // when migrateAgentNames called
    // then key becomes "builder"
    const agents = { "Atlas": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["builder"]).toEqual({ model: "test" })
    expect(migrated["Atlas"]).toBeUndefined()
  })

  test("migrates Plan variants to lowercase", () => {
    // given agents config with "Plan (Planner)" key
    // when migrateAgentNames called
    // then key becomes "planner"
    const agents = { "Plan (Planner)": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["planner"]).toEqual({ model: "test" })
    expect(migrated["Plan (Planner)"]).toBeUndefined()
  })

  test("migrates Analyst variants to lowercase", () => {
    // given agents config with "Analyst (Plan Consultant)" key
    // when migrateAgentNames called
    // then key becomes "analyst"
    const agents = { "Analyst (Plan Consultant)": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["analyst"]).toEqual({ model: "test" })
    expect(migrated["Analyst (Plan Consultant)"]).toBeUndefined()
  })

  test("migrates Metis variants to plan-analyzer", () => {
    // given agents config with "Metis" key
    // when migrateAgentNames called
    // then key becomes "plan-analyzer"
    const agents = { Metis: { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["plan-analyzer"]).toEqual({ model: "test" })
    expect(migrated["Metis"]).toBeUndefined()
  })

  test("migrates Momus variants to plan-reviewer", () => {
    // given agents config with "Momus" key
    // when migrateAgentNames called
    // then key becomes "plan-reviewer"
    const agents = { Momus: { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["plan-reviewer"]).toEqual({ model: "test" })
    expect(migrated["Momus"]).toBeUndefined()
  })

  test("migrates QA variants to lowercase", () => {
    // given agents config with "QA (Plan Reviewer)" key
    // when migrateAgentNames called
    // then key becomes "qa"
    const agents = { "QA (Plan Reviewer)": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["qa"]).toEqual({ model: "test" })
    expect(migrated["QA (Plan Reviewer)"]).toBeUndefined()
  })

  test("migrates Dev-Junior to lowercase", () => {
    // given agents config with "Dev-Junior" key
    // when migrateAgentNames called
    // then key becomes "dev-junior"
    const agents = { "Dev-Junior": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["dev-junior"]).toEqual({ model: "test" })
    expect(migrated["Dev-Junior"]).toBeUndefined()
  })

  test("preserves lowercase passthrough", () => {
    // given agents config with "architect" key
    // when migrateAgentNames called
    // then key remains "architect" (no change needed)
    const agents = { "architect": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(false)
...
describe("migrateHookNames", () => {
  ...
  test("migrates kord-orchestrator to build", () => {
    // given: Config with legacy kord-orchestrator hook
    const hooks = ["kord-orchestrator", "comment-checker"]

    // when: Migrate hook names
    const { migrated, changed, removed } = migrateHookNames(hooks)

    // then: kord-orchestrator should be migrated to build
    expect(changed).toBe(true)
    expect(migrated).toContain("build")
    expect(migrated).toContain("comment-checker")
    expect(migrated).not.toContain("kord-orchestrator")
    expect(removed).toEqual([])
  })
  ...
  test("handles mixed migration and removal", () => {
    ...
    const hooks = ["anthropic-auto-compact", "preemptive-compaction", "kord-orchestrator"]
...
describe("migrateConfigFile", () => {
  ...
  test("migrates omo_agent to kord_agent", () => {
    ...
    // then: omo_agent should be migrated to kord_agent
    expect(needsWrite).toBe(true)
    expect(rawConfig.kord_agent).toEqual({ disabled: false })
  ...
  test("does not write if no migration needed", () => {
   ...
   test("migrates model versions in agents", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         kord: { model: "openai/gpt-5.2-codex", temperature: 0.1 },
       },
     }
     ...
     expect(agents["kord"].model).toBe("openai/gpt-5.3-codex")
   })
   ...
   test("does not set needsWrite when no model versions need migration", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         kord: { model: "openai/gpt-5.3-codex" },
       },
       categories: {
...
describe("migration maps", () => {
  test("AGENT_NAME_MAP contains all expected legacy mappings", () => {
    ...
    expect(AGENT_NAME_MAP["omo"]).toBe("kord")
    expect(AGENT_NAME_MAP["OmO"]).toBe("kord")
    expect(AGENT_NAME_MAP["OmO-Plan"]).toBe("planner")
    expect(AGENT_NAME_MAP["omo-plan"]).toBe("planner")
    expect(AGENT_NAME_MAP["Planner-Kord"]).toBe("planner")
    expect(AGENT_NAME_MAP["plan-consultant"]).toBe("analyst")
    expect(AGENT_NAME_MAP["Metis"]).toBe("plan-analyzer")
    expect(AGENT_NAME_MAP["Momus"]).toBe("plan-reviewer")
  })
...
describe("migrateModelVersions", () => {
  test("replaces old model string in agent config", () => {
    ...
    expect(changed).toBe(true)
    const kord = migrated["kord"] as Record<string, unknown>
    expect(kord.model).toBe("openai/gpt-5.3-codex")
  ...
  test("replaces anthropic model version", () => {
    ...
    expect(changed).toBe(true)
    const plan = migrated["plan"] as Record<string, unknown>
    expect(plan.model).toBe("anthropic/claude-opus-4-6")
  })

  test("leaves unknown model strings untouched", () => {
    // given: Agent config with unknown model
    const agents = {
      architect: { model: "openai/gpt-5.2", temperature: 0.5 },
    }
    ...
    expect(changed).toBe(false)
    const architect = migrated["architect"] as Record<string, unknown>
    expect(architect.model).toBe("openai/gpt-5.2")
  })

  test("handles agent config with no model field", () => {
    // given: Agent config without model field
    const agents = {
      kord: { temperature: 0.1, prompt: "custom" },
    }
  ...
  test("handles agent config with non-string model", () => {
    ...
    const agents = {
      kord: { model: 123, temperature: 0.1 },
    }
  ...
  test("migrates multiple agents in one pass", () => {
    ...
    const agents = {
      kord: { model: "openai/gpt-5.2-codex" },
      plan: { model: "anthropic/claude-opus-4-5" },
      architect: { model: "openai/gpt-5.2" },
    }
    ...
    expect(changed).toBe(true)
    expect((migrated["kord"] as Record<string, unknown>).model).toBe("openai/gpt-5.3-codex")
    expect((migrated["plan"] as Record<string, unknown>).model).toBe("anthropic/claude-opus-4-6")
    expect((migrated["architect"] as Record<string, unknown>).model).toBe("openai/gpt-5.2")
  })
...
describe("migrateConfigFile with backup", () => {
  ...
  test("preserves model setting without auto-conversion to category", () => {
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>
    expect(agents["vision"].model).toBe("anthropic/claude-haiku-4-5")
  ...
  test("preserves category setting when explicitly set", () => {
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>
    expect(agents["vision"].category).toBe("quick")
    expect(agents.architect.category).toBe("ultrabrain")
  })
  ...
  test("does not write when no migration needed", () => {
     ...
     fs.writeFileSync(testConfigPath, globalThis.JSON.stringify({ agents: { kord: { model: "test" } } }, null, 2))
     cleanupPaths.push(testConfigPath)
