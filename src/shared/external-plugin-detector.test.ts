...
describe("external-plugin-detector", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kord-aios-test-"))
  })
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only kord-aios is configured", () => {
      // given - opencode.json with only kord-aios
      const opencodeDir = path.join(tempDir, ".opencode")
      fs.mkdirSync(opencodeDir, { recursive: true })
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kord-aios"] })
      )
      ...
      expect(result.detected).toBe(false)
      expect(result.pluginName).toBeNull()
      expect(result.allPlugins).toContain("kord-aios")
    })
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kord-aios", "opencode-notifier"] })
      )
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kord-aios", "opencode-notifier@1.2.3"] })
      )
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["kord-aios", "@mohak34/opencode-notifier"] })
      )
