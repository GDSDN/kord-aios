/**
 * Detects external plugins that may conflict with Kord AIOS features.
...
/**
 * Known notification plugins that conflict with Kord AIOS session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[kord-aios] External notification plugin detected: ${pluginName}

Both Kord AIOS and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   Kord AIOS session-notification has been auto-disabled.

   To use Kord AIOS notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in kord-aios.json`
}
