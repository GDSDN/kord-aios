...
import {
  ...
  createStartWorkHook,
  createBuildHook,
  createPlanMdOnlyHook,
  createDevNotepadHook,
...
const OhMyOpenCodePlugin: Plugin = async (ctx) => {
  ...
  const autoUpdateChecker = isHookEnabled("auto-update-checker")
    ? createAutoUpdateCheckerHook(ctx, {
        ...
        isKordEnabled: pluginConfig.kord_agent?.disabled !== true,
  ...
  const planMdOnly = isHookEnabled("plan-md-only")
    ? createPlanMdOnlyHook(ctx)
    : null;

  const kordJuniorNotepad = isHookEnabled("dev-junior-notepad")
    ? createDevNotepadHook(ctx)
    : null;
  ...
  const writeExistingFileGuard = isHookEnabled("write-existing-file-guard")
    ...
    : null;
  const agentAuthority = isHookEnabled("agent-authority")
    ? createAgentAuthorityHook(ctx, pluginConfig.agent_authority)
    : null;
  const storyLifecycle = isHookEnabled("story-lifecycle")
    ? createStoryLifecycleHook(ctx, pluginConfig.story_lifecycle)
    : null;
  ...
  const buildHook = isHookEnabled("build")
    ? createBuildHook(ctx, { directory: ctx.directory, backgroundManager })
    : null;
  ...
  const storyReadTool = createStoryReadTool(ctx);
  const storyUpdateTool = createStoryUpdateTool(ctx);
  ...
  const allTools: Record<string, ToolDefinition> = {
    ...builtinTools,
    ...backgroundTools,
    call_kord_agent: callKordAgent,
    ...(lookAt ? { look_at: lookAt } : {}),
    story_read: storyReadTool,
    story_update: storyUpdateTool,
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      await agentAuthority?.["tool.execute.before"]?.(input, output);
      await storyLifecycle?.["tool.execute.before"]?.(input, output);
      await decisionLogger?.["tool.execute.before"]?.(input, output);
      ...
      await planMdOnly?.["tool.execute.before"]?.(input, output);
      await kordJuniorNotepad?.["tool.execute.before"]?.(input, output);
      await buildHook?.["tool.execute.before"]?.(input, output);
    ...
    "tool.execute.after": async (input, output) => {
      ...
      await qualityGate?.["tool.execute.after"]?.(input, output);
      await buildHook?.["tool.execute.after"]?.(input, output);
