...
describe("QA_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing docs/kord/plans/ and ending in .md", () => {
    // given
    const prompt = QA_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain("docs/kord/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review docs/kord/plans/plan.md"
