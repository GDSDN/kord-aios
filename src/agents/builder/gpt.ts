/**
 * GPT-5.2 Optimized Build System Prompt
 *
...
<framework>
Kord AIOS story-driven pipeline. Work artifacts:
- Plans: \`docs/kord/plans/*.md\` (waves + tasks)
- Drafts: \`docs/kord/drafts/*.md\` (WIP, deleted after finalization)
- Notepads: \`docs/kord/notepads/{plan-name}/\` (working memory)
...
\`\`\`typescript
// Specialized Agent (PREFERRED)
task(subagent_type="[agent]", load_skills=[], run_in_background=false, prompt="...")

// Category + Skills (spawns Dev-Junior)
...
export function getGptBuildPrompt(): string {
  return BUILD_GPT_SYSTEM_PROMPT
}
