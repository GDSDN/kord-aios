...
describe("createBuiltinAgents with model overrides", () => {
  test("Kord with default model has thinking config when all models available", async () => {
    // #given
    ...
    try {
      ...
      // #then
      expect(agents.kord.model).toBe("anthropic/claude-opus-4-6")
      expect(agents.kord.thinking).toEqual({ type: "enabled", budgetTokens: 32000 })
      expect(agents.kord.reasoningEffort).toBeUndefined()
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("Kord with GPT model override has reasoningEffort, no thinking", async () => {
    // #given
    const overrides = {
      kord: { model: "github-copilot/gpt-5.2" },
    }
    ...
    // #then
    expect(agents.kord.model).toBe("github-copilot/gpt-5.2")
    expect(agents.kord.reasoningEffort).toBe("medium")
    expect(agents.kord.thinking).toBeUndefined()
  })

  test("Build uses uiSelectedModel when provided", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
    ...
    try {
      ...
      expect(agents.builder.model).toBe("openai/gpt-5.2")
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("Kord is created on first run when no availableModels or cache exist", async () => {
    // #given
   ...
   test("Architect uses connected provider fallback when availableModels is empty and cache exists", async () => {
     ...
     // #then - architect resolves via connected cache fallback to openai/gpt-5.2 (not system default)
   ...
   test("Architect created without model field when no cache exists (first run scenario)", async () => {
  ...
  test("Architect with GPT model override has reasoningEffort, no thinking", async () => {
    // #given
    const overrides = {
  ...
  test("Architect with Claude model override has thinking, no reasoningEffort", async () => {
    // #given
    const overrides = {
   ...
   test("non-model overrides are still applied after factory rebuild", async () => {
     // #given
     const overrides = {
       kord: { model: "github-copilot/gpt-5.2", temperature: 0.5 },
     }
     ...
     // #then
     expect(agents.kord.model).toBe("github-copilot/gpt-5.2")
     expect(agents.kord.temperature).toBe(0.5)
   })
  ...
  test("createBuiltinAgents excludes disabled skills from availableSkills", async () => {
    ...
    // #then
    expect(agents.kord.prompt).not.toContain("playwright")
    expect(agents.kord.prompt).toContain("frontend-ui-ux")
    expect(agents.kord.prompt).toContain("git-master")
  })
})
...
describe("createBuiltinAgents without systemDefaultModel", () => {
   ...
   test("methodology agents are created via fallback chains without systemDefaultModel", async () => {
     // #given
     const cacheSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(["openai"])
     const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
       new Set(["openai/gpt-5.2", "opencode/gpt-5-nano"])
     )
     ...
     try {
       ...
       expect(agents["squad-creator"]).toBeDefined()
     } finally {
       cacheSpy.mockRestore?.()
       fetchSpy.mockRestore?.()
     }
  ...
  test("kord created via connected cache fallback when all providers available", async () => {
    // #given
...
describe("createBuiltinAgents with requiresProvider gating (dev)", () => {
  test("dev is not created when no required provider is connected", async () => {
    // #given - only anthropic models available, not in dev requiresProvider
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
  ...
  test("dev is created when openai provider is connected", async () => {
  ...
  test("dev is created when github-copilot provider is connected", async () => {
  ...
  test("dev is created when opencode provider is connected", async () => {
  ...
  test("dev is created on first run when no availableModels or cache exist", async () => {
    // #given
    const cacheSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(null)
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(new Set())
    ...
    try {
      ...
      // #then
      expect(agents.dev).toBeDefined()
      expect(agents.dev.model).toBe("openai/gpt-5.3-codex")
    } finally {
      cacheSpy.mockRestore()
      fetchSpy.mockRestore()
    }
  ...
  test("dev is created when explicit config provided even if provider unavailable", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["anthropic/claude-opus-4-6"])
    )
    const overrides = {
      dev: { model: "anthropic/claude-opus-4-6" },
    }
...
describe("createBuiltinAgents with requiresAnyModel gating (kord)", () => {
  test("kord is created when at least one fallback model is available", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["anthropic/claude-opus-4-6"])
    )
  ...
  test("kord is created when explicit config provided even if no models available", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(new Set())
    const overrides = {
  ...
  test("kord is not created when no fallback model is available and provider not connected", async () => {
    // #given - only openai/gpt-5.2 available, not in kord fallback chain
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
    ...
    try {
      ...
      // #then
      expect(agents.kord).toBeUndefined()
    } finally {
      fetchSpy.mockRestore()
      cacheSpy.mockRestore()
    }
...
describe("override.category expansion in createBuiltinAgents", () => {
  ...
  test("kord override with category expands category properties", async () => {
    // #given
    const overrides = {
      kord: { category: "ultrabrain" } as any,
    }
    ...
    expect(agents.kord.variant).toBe("xhigh")
  })

  test("builder override with category expands category properties", async () => {
    // #given
    const overrides = {
      builder: { category: "ultrabrain" } as any,
    }
    ...
    // #then - ultrabrain category: model=openai/gpt-5.3-codex, variant=xhigh
    expect(agents.builder).toBeDefined()
    expect(agents.builder.model).toBe("openai/gpt-5.3-codex")
    expect(agents.builder.variant).toBe("xhigh")
  })
