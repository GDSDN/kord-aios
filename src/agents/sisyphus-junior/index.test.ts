...
describe("createSisyphusJuniorAgentWithOverrides", () => {
  ...
  describe("prompt composition", () => {
    test("base prompt uses canonical DEV identity and retains discipline constraints", () => {
      // given
      const override = {};
      ...
      // then
      expect(result.prompt).toContain("DEV - Developer agent for Open-AIOS");
...
describe("buildSisyphusJuniorPrompt", () => {
  test("GPT model prompt contains GPT-5.2 specific sections", () => {
    ...
    expect(prompt).toContain("You are DEV - Developer agent for Open-AIOS");
  ...
  test("Claude model prompt contains Claude-specific sections", () => {
    ...
    expect(prompt).toContain("DEV - Developer agent for Open-AIOS.");
    expect(prompt).not.toContain(
      "Sisyphus-Junior - Focused executor from OhMyOpenCode.",
    );
  });
...
describe("compatibility alias behavior", () => {
  test("legacy Sisyphus-Junior exports remain aliases to DEV implementation", () => {
    expect(createSisyphusJuniorAgentWithOverrides).toBe(
      createDevAgentWithOverrides,
    );
    expect(SISYPHUS_JUNIOR_DEFAULTS).toBe(DEV_DEFAULTS);
    expect(getSisyphusJuniorPromptSource).toBe(getDevPromptSource);
    expect(buildSisyphusJuniorPrompt).toBe(buildDevPrompt);
  });
});
