...
import {
  createDevJuniorAgentWithOverrides,
  DEV_JUNIOR_DEFAULTS,
  getKordJuniorPromptSource,
  buildKordJuniorPrompt,
} from "./index"

describe("createDevJuniorAgentWithOverrides", () => {
  describe("honored fields", () => {
    test("applies model override", () => {
      ...
      // when
      const result = createDevJuniorAgentWithOverrides(override)
  ...
  describe("defaults", () => {
    test("uses default model when no override", () => {
      ...
      // then
      expect(result.model).toBe(DEV_JUNIOR_DEFAULTS.model)
    })
    ...
    test("uses default temperature when no override", () => {
      ...
      // then
      expect(result.temperature).toBe(DEV_JUNIOR_DEFAULTS.temperature)
    })
  })
  ...
  describe("tool safety (task blocked, call_kord_agent allowed)", () => {
    test("task remains blocked, call_kord_agent is allowed via tools format", () => {
      // given
      const override = {
        tools: {
          task: true,
      ...
      if (tools) {
        ...
        // call_kord_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(tools.call_kord_agent).toBe(true)
        expect(tools.read).toBe(true)
      }
      if (permission) {
        expect(permission.task).toBe("deny")
        // call_kord_agent is NOW ALLOWED for subagents to spawn explore/librarian
        expect(permission.call_kord_agent).toBe("allow")
      }
    })
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // when
      const result = createDevJuniorAgentWithOverrides(override as Parameters<typeof createDevJuniorAgentWithOverrides>[0])

      // then - task blocked, but call_kord_agent allowed for explore/librarian spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
  ...
  describe("prompt composition", () => {
    test("base prompt contains discipline constraints", () => {
      ...
      // then
      expect(result.prompt).toContain("Dev-Junior")
      expect(result.prompt).toContain("You work ALONE")
    })
...
describe("getKordJuniorPromptSource", () => {
...
describe("buildKordJuniorPrompt", () => {
  test("GPT model prompt contains GPT-5.2 specific sections", () => {
    // given
    const model = "openai/gpt-5.2"

    // when
    const prompt = buildKordJuniorPrompt(model, false)
  ...
  test("useTaskSystem=true includes Task_Discipline for GPT", () => {
    ...
    // when
    const prompt = buildKordJuniorPrompt(model, true)
