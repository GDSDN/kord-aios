...
export function buildToolSelectionTable(
  ...
  rows.push("")
  rows.push("**Default flow**: explore/librarian (background) + tools → architect (if required)")
...
export function buildArchitectSection(agents: AvailableAgent[]): string {
  const architectAgent = agents.find((a) => a.name === "architect")
  if (!architectAgent) return ""

  const useWhen = architectAgent.metadata.useWhen || []
  const avoidWhen = architectAgent.metadata.avoidWhen || []
...
export function buildUltraworkSection(
  ...
  if (agents.length > 0) {
    const ultraworkAgentPriority = ["explore", "librarian", "planner", "architect"]
