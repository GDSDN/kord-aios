...
/**
 * KORD-WORKER - The Autonomous Deep Worker
 *
 * Canonical Open-AIOS worker implementation.
 * Optimized for goal-oriented execution with deep exploration and verification.
 */

function buildKordWorkerPrompt(
  ...
  return `You are KORD-WORKER, an autonomous deep worker for software engineering.
...
- Deep worker never self-approves story completion.
- Return implementation and verification evidence to BUILD/BUILD-LOOP orchestration for QA gate and final completion decision.
- If assigned scope conflicts with story/plan/architecture artifacts, escalate to orchestrator specialists; do not silently patch scope.
...
**Artifact Drift Override (higher priority than best-guess):**
- If artifacts disagree materially (story AC vs code reality, PRD vs architecture constraints, or plan vs implementation contract), do NOT choose scope unilaterally.
- Escalate through orchestrator path:
  - planning/story drift -> \`plan\`/\`sm\`
  - product/scope drift -> \`pm\`/\`po\`
  - architecture/technical drift -> \`architect\`
- Continue only on explicitly confirmed scope.
...
export function createKordWorkerAgent(
  model: string,
  availableAgents?: AvailableAgent[],
  ...
  const prompt = availableAgents
    ...
    : buildKordWorkerPrompt([], tools, skills, categories, useTaskSystem);

  return {
    description:
      "Autonomous Deep Worker for Open-AIOS (KORD-WORKER). Goal-oriented execution with GPT 5.2 Codex, deep exploration before action, and end-to-end completion.",
    mode: MODE,
    model,
    maxTokens: 32000,
    prompt,
...
createKordWorkerAgent.mode = MODE;

export const createHephaestusAgent = createKordWorkerAgent;
