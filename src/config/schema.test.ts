...
describe("disabled_hooks schema", () => {
  test("accepts story-workflow-enforcer hook name", () => {
    const config = {
      disabled_hooks: ["story-workflow-enforcer"],
    };
...
describe("Dev agent override", () => {
  test("schema accepts canonical agents['dev'] key", () => {
    // given
    const config = {
      agents: {
        dev: {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.agents?.dev).toBeDefined();
      expect(result.data.agents?.dev?.model).toBe("openai/gpt-5.2");
      expect(result.data.agents?.dev?.temperature).toBe(0.2);
    }
  });

  test("schema accepts legacy agents['sisyphus-junior'] key for compatibility", () => {
    // given
    const config = {
      agents: {
        "sisyphus-junior": {
