...
export function createAutoUpdateCheckerHook(ctx: PluginInput, options: AutoUpdateCheckerOptions = {}) {
  const { showStartupToast = true, isKordEnabled = false, autoUpdate = true } = options
  ...
  return {
    event: ({ event }: { event: { type: string; properties?: unknown } }) => {
      ...
      setTimeout(async () => {
        ...
        if (localDevVersion) {
          if (showStartupToast) {
            showLocalDevToast(ctx, displayVersion, isKordEnabled).catch(() => {})
          }
...
async function showSpinnerToast(ctx: PluginInput, version: string, message: string): Promise<void> {
  ...
  for (let i = 0; i < totalFrames; i++) {
    const spinner = KORD_SPINNER[i % KORD_SPINNER.length]
    await ctx.client.tui
      .showToast({
        body: {
...
async function showLocalDevToast(ctx: PluginInput, version: string | null, isKordEnabled: boolean): Promise<void> {
