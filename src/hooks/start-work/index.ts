...
function extractUserRequestPlanName(promptText: string): string | null {
  ...
  const cleanedArg = rawArg.replace(KEYWORD_PATTERN, "").replace(/--squad\s*=?\s*\S+/gi, "").replace(/--wave\s*=?\s*\d+/gi, "").trim()
  return cleanedArg || null
}

function extractSquadParam(promptText: string): string | undefined {
  const userRequestMatch = promptText.match(/<user-request>\s*([\s\S]*?)\s*<\/user-request>/i)
  if (!userRequestMatch) return undefined
  
  const squadMatch = userRequestMatch[1].match(/--squad\s*=?\s*(\S+)/i)
  return squadMatch?.[1] ?? undefined
}

function extractWaveParam(promptText: string): number | undefined {
  const userRequestMatch = promptText.match(/<user-request>\s*([\s\S]*?)\s*<\/user-request>/i)
  if (!userRequestMatch) return undefined
  
  const waveMatch = userRequestMatch[1].match(/--wave\s*=?\s*(\d+)/i)
  return waveMatch ? parseInt(waveMatch[1], 10) : undefined
}

function detectPlanType(planPath: string): "story-driven" | "task-driven" | undefined {
  const waves = parsePlanWaves(planPath)
  if (waves.length > 0) return "story-driven"
  return undefined
}
...
export function createStartWorkHook(ctx: PluginInput) {
  return {
    "chat.message": async (
      ...
      // NOT on description text like "Start Kord work session from Plan plan"
      ...
      const squad = extractSquadParam(promptText)
      const wave = extractWaveParam(promptText)
      ...
      if (explicitPlanName) {
        ...
        const allPlans = findPlanPlans(ctx.directory)
...
**Started**: ${timestamp}${squad ? `\n**Squad**: ${squad}` : ""}${wave ? `\n**Wave**: Starting at Wave ${wave}` : ""}
...
Looking for new plans...`
      ...
      if ((!existingState && !explicitPlanName) || (existingState && !explicitPlanName && getPlanProgress(existingState.active_plan).isComplete)) {
        const plans = findPlanPlans(ctx.directory)
...
No Plan plan files found at docs/kord/plans/
