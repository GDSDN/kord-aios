...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should skip ultrawork injection when agent name contains 'planner'", async () => {
    ...
    // when - ultrawork keyword detected with planner agent
    await hook["chat.message"]({ sessionID, agent: "Plan (Planner)" }, output)
  ...
  test("should use normal ultrawork message when agent is Kord", async () => {
    // given - collector and Kord agent
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    const sessionID = "kord-session"
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork implement this feature" }],
    }

    // when - ultrawork keyword detected with Kord agent
    await hook["chat.message"]({ sessionID, agent: "kord" }, output)
  ...
  test("should skip ultrawork for plan but inject for kord", async () => {
    // given - two sessions, one with plan, one with kord
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    // Second session with kord
    const kordSessionID = "kord-second"
    const kordOutput = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork implement" }],
    }
    await hook["chat.message"]({ sessionID: kordSessionID, agent: "kord" }, kordOutput)

    // then - planner should have no injection, kord should have normal ultrawork
    const plannerTextPart = plannerOutput.parts.find(p => p.type === "text")
    expect(plannerTextPart!.text).toBe("ultrawork plan")

    const kordTextPart = kordOutput.parts.find(p => p.type === "text")
    expect(kordTextPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(kordTextPart!.text).toContain("---")
    expect(kordTextPart!.text).toContain("implement")
  })

  test("should use session state agent over stale input.agent (bug fix)", async () => {
    // given - same session, agent switched from planner to kord in session state
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    const sessionID = "same-session-agent-switch"

    // Simulate: session state was updated to kord (by index.ts updateSessionAgent)
    ...
    // when - hook receives stale input.agent="planner" but session state says "Kord"
    await hook["chat.message"]({ sessionID, agent: "planner" }, output)

    // then - should use Kord from session state, NOT planner from stale input
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
    expect(textPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(textPart!.text).not.toContain("YOU ARE A PLANNER, NOT AN IMPLEMENTER")
  ...
  test("should fall back to input.agent when session state is empty and skip ultrawork for plan", async () => {
