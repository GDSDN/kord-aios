...
describe("keyword-detector star command detection", () => {
  let logCalls: Array<{ msg: string; data?: unknown }>;
  let logSpy: ReturnType<typeof spyOn>;
  ...
  test("should detect star command at start of message", async () => {
    // given - message starts with star command
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-star";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "*code-review please review this code" }],
    };
  ...
  test("should normalize star command to kebab-case", async () => {
    // given - message with mixed-case star command
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-normalize";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "*Code_Review Do my bidding" }],
    };

    // when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // then - command should be normalized to kebab-case
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain("code-review");
    expect(textPart!.text).toContain('skill(name="code-review")');
  ...
  test("should NOT detect star command when not at start", async () => {
    // given - message has asterisk but not at start
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-no-star";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "Please run mytask command" }],
    };

    // when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // then - star command advisory should NOT be added
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
  ...
  test("should NOT detect star command in normal text", async () => {
    // given - normal message without star command
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-normal";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "just a normal message without stars" }],
    };
    ...
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toBe("just a normal message without stars");
  });

  test("should handle star command with whitespace", async () => {
    // given - message with star command followed by whitespace
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-whitespace";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "*git-master   execute git master" }],
    };

    // when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // then - star command should be detected with trimmed command
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
    expect(textPart!.text).toContain("git-master");
    expect(textPart!.text).toContain("execute git master");
  });

  test("should handle star command without trailing text", async () => {
    // given - message is just a star command
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-only";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "*playwright" }],
    };

    // when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // then - star command should be detected
    const textPart = output.parts.find((p) => p.type === "text");
    expect(textPart).toBeDefined();
  ...
  test("should log star command detection", async () => {
    // given - message with star command
    const collector = new ContextCollector();
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector);
    const sessionID = "test-session-log";
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "*search-npm find packages" }],
    };

    // when - keyword detection runs
    await hook["chat.message"]({ sessionID }, output);

    // then - log should contain star command detection
    const starLog = logCalls.find((c) =>
      c.msg.includes("Star command detected"),
    );
    expect(starLog).toBeDefined();
    expect(starLog!.data).toEqual({
      originalCommand: "search-npm",
      normalizedName: "search-npm",
