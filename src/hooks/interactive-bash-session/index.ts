...
import { KORD_SESSION_PREFIX, buildSessionReminderMessage } from "./constants";
 ...
 * e.g., "kord-x:1" -> "kord-x", "kord-x:1.2" -> "kord-x"
 */
 ...
 * e.g., `tmux -L socket-name new-session -s kord-x`
...
export function createInteractiveBashSessionHook(ctx: PluginInput) {
  ...
  function isKordSession(sessionName: string | null): boolean {
    return sessionName !== null && sessionName.startsWith(KORD_SESSION_PREFIX);
  }
  ...
  const toolExecuteAfter = async (
    ...
    if (isNewSession && isKordSession(sessionName)) {
      state.tmuxSessions.add(sessionName!);
      stateChanged = true;
    } else if (isKillSession && isKordSession(sessionName)) {
