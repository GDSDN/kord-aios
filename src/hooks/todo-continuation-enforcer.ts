...
const DEFAULT_SKIP_AGENTS = ["plan", "compaction"]
...
export function createTodoContinuationEnforcer(
  ...
  function cancelCountdown(sessionID: string): void {
    ...
    if (state.countdownTimer) {
      timers.clearTimeout(state.countdownTimer)
      state.countdownTimer = undefined
    }
    if (state.countdownInterval) {
      timers.clearInterval(state.countdownInterval)
...
${todoList}`
  ...
  function startCountdown(
    ...
    state.countdownStartedAt = timers.now()

    state.countdownInterval = timers.setInterval(() => {
      secondsRemaining--
    ...
    state.countdownTimer = timers.setTimeout(() => {
      cancelCountdown(sessionID)
  ...
  const handler = async ({ event }: { event: { type: string; properties?: unknown } }): Promise<void> => {
    ...
    if (event.type === "session.error") {
      ...
      if (error?.name === "MessageAbortedError" || error?.name === "AbortError") {
        const state = getState(sessionID)
        state.abortDetectedAt = timers.now()
    ...
    if (event.type === "session.idle") {
      ...
      // Check 1: Event-based abort detection (primary, most reliable)
        ...
        const timeSinceAbort = timers.now() - state.abortDetectedAt
    ...
    if (event.type === "message.updated") {
      ...
      if (role === "user") {
          ...
          const elapsed = timers.now() - state.countdownStartedAt
