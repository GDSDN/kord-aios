...
function createTestTimers(): TestTimers {
  ...
  const setTimeoutImpl = ((callback: TimerCallback, delay?: number, ...args: any[]) => {
    return schedule(callback, delay, null, args) as unknown as ReturnType<typeof setTimeout>
  }) as typeof setTimeout

  const setIntervalImpl = ((callback: TimerCallback, delay?: number, ...args: any[]) => {
  ...
  const clearIntervalImpl = ((id?: number) => {
    clear(id)
...
describe("todo-continuation-enforcer", () => {
  ...
  function createHook(
    ...
    options: Parameters<typeof createTodoContinuationEnforcer>[1] = {},
  ) {
    return createTodoContinuationEnforcer(input, {
      ...options,
  ...
  test("should inject continuation when idle with incomplete todos", async () => {
    ...
    await timers.advanceBy(2500, true)
  ...
  test("should cancel countdown on user message after grace period", async () => {
    ...
    await timers.advanceBy(600, true)
    await hook.handler({
      event: {
  ...
  test("should inject after recovery complete", async () => {
    ...
    await timers.advanceBy(3000, true)
  ...
  test("should accept skipAgents option without error", async () => {
    // given - session with skipAgents configured for Plan
    const sessionID = "main-plan-option"
    setMainSession(sessionID)

    // when - create hook with skipAgents option (should not throw)
    const hook = createHook(createMockPluginInput(), {
      skipAgents: ["Plan (Planner)", "custom-agent"],
    })
  ...
  test("should inject when abort flag is stale (>3s old)", async () => {
    ...
    await timers.advanceBy(3100, true)
  ...
  test("should extract model from assistant message with flat modelID/providerID", async () => {
    ...
    const mockMessagesWithAssistant = [
      { info: { id: "msg-1", role: "user", agent: "kord", model: { providerID: "openai", modelID: "gpt-5.2" } } },
      { info: { id: "msg-2", role: "assistant", agent: "kord", modelID: "gpt-5.2", providerID: "openai" } },
    ]
  ...
  test("should skip compaction agent messages when resolving agent info", async () => {
    // given - session where last message is from compaction agent but previous was Kord
    const sessionID = "main-compaction-filter"
    setMainSession(sessionID)

    const mockMessagesWithCompaction = [
      { info: { id: "msg-1", role: "user", agent: "kord", model: { providerID: "anthropic", modelID: "claude-sonnet-4-5" } } },
      { info: { id: "msg-2", role: "assistant", agent: "kord", modelID: "claude-sonnet-4-5", providerID: "anthropic" } },
    ...
    // then - continuation uses Kord (skipped compaction agent)
    expect(promptCalls.length).toBe(1)
    expect(promptCalls[0].agent).toBe("kord")
  })
  ...
  test("should skip injection when plan agent is after compaction", async () => {
    // given - plan session that was compacted
    const sessionID = "main-plan-compacted"
    setMainSession(sessionID)

    const mockMessagesPlanCompacted = [
      { info: { id: "msg-1", role: "user", agent: "plan" } },
      { info: { id: "msg-2", role: "assistant", agent: "plan" } },
    ...
    const mockInput = {
      client: {
        session: {
          ...
          messages: async () => ({ data: mockMessagesPlanCompacted }),
          prompt: async (opts: any) => {
            promptCalls.push({
              sessionID: opts.path.id,
              agent: opts.body.agent,
    ...
    // then - no continuation (plan found after filtering compaction, plan is in skipAgents)
    expect(promptCalls).toHaveLength(0)
  })
