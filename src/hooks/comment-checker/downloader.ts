...
/**
 * Get the cache directory for Kord AIOS binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "kord-aios", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[kord-aios] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[kord-aios] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[kord-aios] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[kord-aios] Comment checking disabled.`)
    return null
  }
}
