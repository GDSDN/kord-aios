...
├── plan-md-only/         # Planner read-only mode
...
├── start-work/                 # Kord work session starter
...
├── dev-junior-notepad/    # Dev Junior notepad
...
- **PreToolUse**: subagentQuestionBlocker → questionLabelTruncator → claudeCodeHooks → nonInteractiveEnv → commentChecker → directoryAgentsInjector → directoryReadmeInjector → rulesInjector → planMdOnly → kordJuniorNotepad → writeExistingFileGuard → buildHook
- **PostToolUse**: claudeCodeHooks → toolOutputTruncator → contextWindowMonitor → commentChecker → directoryAgentsInjector → directoryReadmeInjector → rulesInjector → emptyTaskResponseDetector → agentUsageReminder → interactiveBashSession → editErrorRecovery → delegateTaskRetry → buildHook → taskResumeInfo → taskReminder
