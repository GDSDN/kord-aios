...
describe("findRuleFiles", () => {
  ...
  describe(".github/instructions/ discovery", () => {
    ...
    it("should ignore non-.instructions.md files in .github/instructions/", () => {
      ...
      expect(paths.some((p) => p.endsWith("invalid.md") && p.includes(join(".github", "instructions")))).toBe(false);
  ...
  describe("backward compatibility", () => {
    it("should still discover .claude/rules/ files", () => {
      ...
      const paths = candidates.map((c) => c.path);
      expect(paths.some((p) => p.includes(join(".claude", "rules")))).toBe(true);
    });
    ...
    it("should still discover .cursor/rules/ files", () => {
      ...
      const paths = candidates.map((c) => c.path);
      expect(paths.some((p) => p.includes(join(".cursor", "rules")))).toBe(true);
    });
  ...
  describe("mixed sources", () => {
    it("should discover rules from all sources", () => {
      ...
      expect(paths.some((p) => p.includes(join(".claude", "rules")))).toBe(true);
      expect(paths.some((p) => p.includes(join(".cursor", "rules")))).toBe(true);
      expect(paths.some((p) => p.includes(join(".github", "instructions")))).toBe(
        true
      );
...
describe("findProjectRoot", () => {
  ...
  it("should return null when no project markers found", () => {
    ...
    // then should return null, or if a parent dir has markers (e.g. .git in home dir on Windows),
    // the result must be outside TEST_DIR (not our isolated test structure)
    if (root === null) {
      expect(root).toBeNull();
    } else {
      expect(root.startsWith(TEST_DIR)).toBe(false);
    }
  });
});
