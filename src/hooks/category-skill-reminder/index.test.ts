...
describe("category-skill-reminder hook", () => {
  ...
  describe("target agent detection", () => {
    test("should inject reminder for kord agent after 3 tool calls", async () => {
      // given - kord agent session with multiple tool calls
    ...
    test("should inject reminder for build agent", async () => {
      // given - build agent session
      const hook = createHook()
      const sessionID = "build-session"
      updateSessionAgent(sessionID, "build")
    ...
    test("should inject reminder for dev-junior agent", async () => {
      // given - dev-junior agent session
      const hook = createHook()
      const sessionID = "junior-session"
      updateSessionAgent(sessionID, "dev-junior")
    ...
    test("should detect agent from input.agent when session state is empty", async () => {
      ...
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "1", agent: "Kord" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "2", agent: "Kord" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "3", agent: "Kord" }, output)
  ...
  describe("delegation tool tracking", () => {
    test("should NOT inject reminder if task is used", async () => {
      // given - kord agent that uses task
      const hook = createHook()
    ...
    test("should NOT inject reminder if call_kord_agent is used", async () => {
      // given - kord agent that uses call_kord_agent
      const hook = createHook()
      const sessionID = "kord-agent-session"
      updateSessionAgent(sessionID, "Kord")

      const output = { title: "", output: "result", metadata: {} }

      // when - call_kord_agent is used first
      await hook["tool.execute.after"]({ tool: "call_kord_agent", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "3" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "4" }, output)
    ...
    test("should NOT inject reminder if task tool is used", async () => {
      // given - kord agent that uses task tool
      const hook = createHook()
  ...
  describe("tool call counting", () => {
    test("should NOT inject reminder before 3 tool calls", async () => {
      // given - kord agent with only 2 tool calls
      const hook = createHook()
    ...
    test("should only count delegatable work tools", async () => {
      // given - kord agent with mixed tool calls
      const hook = createHook()
  ...
  describe("event handling", () => {
    test("should reset state on session.deleted event", async () => {
      // given - kord agent with reminder already shown
      const hook = createHook()
  ...
  describe("case insensitivity", () => {
    test("should handle tool names case-insensitively", async () => {
      // given - kord agent with mixed case tool names
      const hook = createHook()
    ...
    test("should handle delegation tool names case-insensitively", async () => {
      // given - kord agent using TASK in uppercase
      const hook = createHook()
