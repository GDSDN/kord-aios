...
export async function executeCompact(
  ...
  timers?: { now?: () => number; setTimeout?: typeof setTimeout },
): Promise<void> {
  const clockNow = timers?.now ?? Date.now
  const scheduleTimeout = timers?.setTimeout ?? globalThis.setTimeout
  ...
  try {
    ...
    if (errorData?.errorType?.includes("non-empty content")) {
     ...
     if (clockNow() - retryState.lastAttemptTime > 300000) {
    ...
    if (retryState.attempt < RETRY_CONFIG.maxAttempts) {
      retryState.attempt++;
       retryState.lastAttemptTime = clockNow();
