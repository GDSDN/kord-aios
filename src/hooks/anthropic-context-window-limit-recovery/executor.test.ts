...
function createFakeTimeouts(): FakeTimeouts {
  ...
  const restore = () => {
    // no-op (we do not patch globals)
  }

  return { now: () => now, setTimeout: setTimeoutImpl, advanceBy }
}
...
describe("executeCompact lock management", () => {
  ...
  test("clears lock on successful summarize completion", async () => {
    ...
    await executeCompact(sessionID, msg, autoCompactState, mockClient, directory, undefined, {
      now: fakeTimeouts.now,
      setTimeout: fakeTimeouts.setTimeout,
    })
