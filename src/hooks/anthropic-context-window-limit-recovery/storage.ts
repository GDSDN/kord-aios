...
type TruncationDeps = {
  findToolResultsBySize: (sessionID: string) => ToolResultInfo[]
  truncateToolResult: (partPath: string) => { success: boolean; toolName?: string; originalSize?: number }
}
...
export function truncateUntilTargetTokens(
  ...
  const helpers: TruncationDeps = deps ?? {
  ...
  const results = helpers.findToolResultsBySize(sessionID)
  ...
  for (const result of results) {
    const truncateResult = helpers.truncateToolResult(result.partPath)
