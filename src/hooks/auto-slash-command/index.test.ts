...
describe("createAutoSlashCommandHook", () => {
  ...
  describe("slash command replacement", () => {
    ...
    it("should replace exact star command with skill template", async () => {
      const hook = createAutoSlashCommandHook({
        skills: [createMockSkill("develop-story", "Execute story development")],
      });
      const sessionID = `test-session-star-exact-${Date.now()}`;
      const input = createMockInput(sessionID);
      const output = createMockOutput("*develop-story");

      await hook["chat.message"](input, output);

      expect(output.parts[0].text).toContain("<auto-slash-command>");
      expect(output.parts[0].text).toContain("# *develop-story Command");
    });

    it("should replace unknown star command with skill suggestions", async () => {
      const hook = createAutoSlashCommandHook({
        skills: [
          createMockSkill("develop-story"),
          createMockSkill("create-next-story"),
        ],
      });
      const sessionID = `test-session-star-fallback-${Date.now()}`;
      const input = createMockInput(sessionID);
      const output = createMockOutput("*story");

      await hook["chat.message"](input, output);

      expect(output.parts[0].text).toContain("<auto-slash-command>");
      expect(output.parts[0].text).toContain("did not match an exact skill");
      expect(output.parts[0].text).toContain("*develop-story");
    });
  });
