...
After`;
  ...
  describe("parseSlashCommand", () => {
    ...
    it("should parse star command as skill intent", () => {
      const text = "*develop-story";

      const result = parseSlashCommand(text);

      expect(result).not.toBeNull();
      expect(result?.prefix).toBe("*");
      expect(result?.command).toBe("develop-story");
      expect(result?.args).toBe("");
    });

    it("should normalize underscores in star command names", () => {
      const text = "*develop_story";
  ...
  describe("detectSlashCommand", () => {
    ...
    it("should detect star command in plain text", () => {
      const text = "*create-next-story scope the backlog";

      const result = detectSlashCommand(text);

      expect(result).not.toBeNull();
      expect(result?.prefix).toBe("*");
      expect(result?.command).toBe("create-next-story");
      expect(result?.args).toBe("scope the backlog");
    });
  ...
  describe("extractPromptText", () => {
    ...
    it("should prioritize a star command part when present", () => {
      const parts = [
        { type: "text", text: "Context first" },
        { type: "text", text: "*develop-story" },
      ];

      const result = extractPromptText(parts);

      expect(result).toBe("*develop-story");
    });
