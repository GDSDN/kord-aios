...
export function createAutoSlashCommandHook(
  ...
  return {
    "chat.message": async (
      ...
      // Debug logging to diagnose command preprocessing issues
      if (promptText.startsWith("/") || promptText.startsWith("*")) {
        log(`[auto-slash-command] chat.message hook received command`, {
      ...
      const commandKey = `${input.sessionID}:${input.messageID}:${parsed.prefix}:${parsed.command}`;
      if (sessionProcessedCommands.has(commandKey)) {
        return;
      }
      sessionProcessedCommands.add(commandKey);

      log(`[auto-slash-command] Detected: ${parsed.prefix}${parsed.command}`, {
        sessionID: input.sessionID,
        args: parsed.args,
      });
