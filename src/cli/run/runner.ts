...
const normalizeAgentName = (agent?: string): string | undefined => {
  ...
  // Try migration map first (handles legacy names like "Atlas" -> "builder")
  const migrated = AGENT_NAME_MAP[trimmed] ?? AGENT_NAME_MAP[trimmed.toLowerCase()]
...
const isAgentDisabled = (agent: string, config: OhMyOpenCodeConfig): boolean => {
  ...
  if (lowered === "kord" && config.kord_agent?.disabled === true) {
    return true
  }
