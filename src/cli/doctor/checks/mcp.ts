...
import { detectMcpConfiguration } from "../../mcp/config-detector";
...
export async function checkRecommendedMcpServers(): Promise<CheckResult> {
  const detection = detectMcpConfiguration();
  const missing = detection.missingRecommendedMcpIds;

  if (missing.length > 0) {
    return {
      name: CHECK_NAMES[CHECK_IDS.MCP_RECOMMENDED],
      status: "warn",
      message: `${missing.length} recommended MCP server(s) missing`,
      details: [
        `Missing: ${missing.join(", ")}`,
        `Configured: ${detection.configuredMcpIds.join(", ") || "none"}`,
      ],
    };
  }
  ...
  return {
    name: CHECK_NAMES[CHECK_IDS.MCP_RECOMMENDED],
    status: "pass",
    message: "All recommended MCP servers configured",
    details: [`Configured: ${detection.configuredMcpIds.join(", ") || "none"}`],
  };
}
...
export function getMcpCheckDefinitions(): CheckDefinition[] {
  return [
    ...
    {
      id: CHECK_IDS.MCP_RECOMMENDED,
      name: CHECK_NAMES[CHECK_IDS.MCP_RECOMMENDED],
      category: "tools",
      check: checkRecommendedMcpServers,
      critical: false,
    },
  ];
}
