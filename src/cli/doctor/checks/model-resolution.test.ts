...
describe("model-resolution check", () => {
  describe("getModelResolutionInfo", () => {
    ...
    it("returns agent requirements with provider chains", async () => {
      ...
      const kord = info.agents.find((a) => a.name === "kord")
      expect(kord).toBeDefined()
      expect(kord!.requirement.fallbackChain[0]?.model).toBe("claude-opus-4-6")
      expect(kord!.requirement.fallbackChain[0]?.providers).toContain("anthropic")
    })
  ...
  describe("getModelResolutionInfoWithOverrides", () => {
    // given: User has overrides in kord-aios.json
    ...
    it("shows user override for agent when configured", async () => {
      ...
      const architect = info.agents.find((a) => a.name === "architect")
      expect(architect).toBeDefined()
      expect(architect!.userOverride).toBe("anthropic/claude-opus-4-6")
      expect(architect!.effectiveResolution).toBe("User override: anthropic/claude-opus-4-6")
    })
    ...
    it("shows provider fallback when no override exists", async () => {
      ...
      const kord = info.agents.find((a) => a.name === "kord")
      expect(kord).toBeDefined()
      expect(kord!.userOverride).toBeUndefined()
      expect(kord!.effectiveResolution).toContain("Provider fallback:")
      expect(kord!.effectiveResolution).toContain("anthropic")
    })
    ...
    it("captures user variant for agent when configured", async () => {
      ...
      const mockConfig = {
        agents: {
          architect: { model: "openai/gpt-5.2", variant: "xhigh" },
        },
      }
      ...
      const architect = info.agents.find((a) => a.name === "architect")
      expect(architect).toBeDefined()
      expect(architect!.userOverride).toBe("openai/gpt-5.2")
