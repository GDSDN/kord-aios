...
import * as configDetector from "../../mcp/config-detector";
...
describe("mcp check", () => {
  ...
  describe("checkRecommendedMcpServers", () => {
    ...
    it("returns warn when recommended MCP servers are missing", async () => {
      detectSpy = spyOn(
        configDetector,
        "detectMcpConfiguration",
      ).mockReturnValue({
        configuredMcpIds: ["playwright"],
        recommendedMcpIds: [
          "playwright",
          "desktop-commander",
          "context7",
        ...
        missingRecommendedMcpIds: ["desktop-commander", "context7", "exa"],
        scannedFiles: [],
      });

      const result = await mcp.checkRecommendedMcpServers();

      expect(result.status).toBe("warn");
      expect(result.message).toContain("3");
      expect(result.details?.some((d) => d.includes("desktop-commander"))).toBe(
        true,
      );
    });

    it("returns pass when all recommended MCP servers are configured", async () => {
      detectSpy = spyOn(
        configDetector,
        "detectMcpConfiguration",
      ).mockReturnValue({
      ...
      expect(result.status).toBe("pass");
      expect(result.message).toContain("All recommended");
    });
  });

  describe("getMcpCheckDefinitions", () => {
    it("returns definitions for builtin, user, and recommended", () => {
      // given
      ...
      expect(defs.map((d) => d.id)).toContain("mcp-recommended");
    });
  });
});
