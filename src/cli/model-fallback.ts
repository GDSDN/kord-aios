...
const SCHEMA_URL = "https://raw.githubusercontent.com/GDSDN/kord-aios/master/assets/kord-opencode.schema.json"
...
function getKordFallbackChain(): FallbackEntry[] {
  return AGENT_MODEL_REQUIREMENTS.kord.fallbackChain
}
...
export function generateModelConfig(config: InstallConfig): GeneratedOmoConfig {
    ...
    return {
      ...
      agents: Object.fromEntries(
        Object.entries(AGENT_MODEL_REQUIREMENTS)
          .filter(([role, req]) => !(role === "kord" && req.requiresAnyModel))
  ...
  for (const [role, req] of Object.entries(AGENT_MODEL_REQUIREMENTS)) {
    ...
    if (role === "kord") {
      const fallbackChain = getKordFallbackChain()
