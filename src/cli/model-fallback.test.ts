...
describe("generateModelConfig", () => {
  ...
  describe("single native provider", () => {
    ...
    test("uses Claude models with isMax20 flag", () => {
      ...
      // #then should use higher capability models for Kord
      expect(result).toMatchSnapshot()
    })
  ...
  describe("Kord agent special cases", () => {
    test("Kord is created when at least one fallback provider is available (Claude)", () => {
      // #given
      const config = createConfig({ hasClaude: true, isMax20: true })
      ...
      // #then
      expect(result.agents?.kord?.model).toBe("anthropic/claude-opus-4-6")
    })

    test("Kord is created when multiple fallback providers are available", () => {
      // #given
      const config = createConfig({
        hasClaude: true,
    ...
    test("Kord is omitted when no fallback provider is available (OpenAI not in chain)", () => {
      // #given
      const config = createConfig({ hasOpenAI: true })
      ...
      // #then
      expect(result.agents?.kord).toBeUndefined()
    })
  })

  describe("Dev agent special cases", () => {
    test("Dev is created when OpenAI is available (openai provider connected)", () => {
      // #given
      const config = createConfig({ hasOpenAI: true })
      ...
      // #then
      expect(result.agents?.dev?.model).toBe("openai/gpt-5.3-codex")
      expect(result.agents?.dev?.variant).toBe("medium")
    })

    test("Dev is created when Copilot is available (github-copilot provider connected)", () => {
      // #given
      const config = createConfig({ hasCopilot: true })
      ...
      // #then
      expect(result.agents?.dev?.model).toBe("github-copilot/gpt-5.3-codex")
      expect(result.agents?.dev?.variant).toBe("medium")
    })

    test("Dev is created when OpenCode Zen is available (opencode provider connected)", () => {
      // #given
      const config = createConfig({ hasOpencodeZen: true })
      ...
      // #then
      expect(result.agents?.dev?.model).toBe("opencode/gpt-5.3-codex")
      expect(result.agents?.dev?.variant).toBe("medium")
    })

    test("Dev is omitted when only Claude is available (no required provider connected)", () => {
      // #given
      const config = createConfig({ hasClaude: true })
      ...
      // #then
      expect(result.agents?.dev).toBeUndefined()
    })

    test("Dev is omitted when only Gemini is available (no required provider connected)", () => {
      // #given
      const config = createConfig({ hasGemini: true })
    ...
    test("Dev is omitted when only ZAI is available (no required provider connected)", () => {
      // #given
      const config = createConfig({ hasZaiCodingPlan: true })
  ...
  describe("schema URL", () => {
    test("always includes correct schema URL", () => {
      ...
      expect(result.$schema).toBe(
        "https://raw.githubusercontent.com/GDSDN/kord-aios/master/assets/kord-opencode.schema.json"
      )
    })
  })
})
