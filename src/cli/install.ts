...
function formatConfigSummary(config: InstallConfig): string {
  ...
  lines.push(formatProvider("OpenAI/ChatGPT", config.hasOpenAI, "GPT-5.2 for Architect"))
  lines.push(formatProvider("Gemini", config.hasGemini))
  lines.push(formatProvider("GitHub Copilot", config.hasCopilot, "fallback"))
  lines.push(formatProvider("OpenCode Zen", config.hasOpencodeZen, "opencode/ models"))
  lines.push(formatProvider("Z.ai Coding Plan", config.hasZaiCodingPlan, "Librarian/Multimodal"))
  lines.push(formatProvider("Kimi For Coding", config.hasKimiForCoding, "Kord/Plan fallback"))
...
function printHeader(isUpdate: boolean): void {
  ...
  console.log()
  console.log(color.bgMagenta(color.white(` Kord AIOS ${mode} `)))
  console.log()
}
...
function getMaturityMessage(status: ProjectMaturityStatus): string {
  switch (status) {
    case "fresh":
      return "New installation detected"
    case "partial":
      return "Incomplete installation detected — resuming setup"
    case "existing":
      return "Existing installation detected — update mode"
  }
}
...
async function runNonTuiInstall(args: InstallArgs): Promise<number> {
  ...
  const version = await getKordAiosVersion()
  if (!installed) {
  ...
  printStep(step++, totalSteps, "Adding Kord AIOS plugin...")
  ...
  printStep(step++, totalSteps, "Writing Kord AIOS configuration...")
  const kordAiosResult = writeKordAiosConfig(config)
  if (!kordAiosResult.success) {
    printError(`Failed: ${kordAiosResult.error}`)
    return 1
  }
  ...
  if (!args.skipDoctor) {
    ...
    console.log()
    console.log(color.bold(color.white("Post-Install Verification:")))
    for (const check of doctorResult.checks) {
      const icon = check.passed ? SYMBOLS.check : SYMBOLS.cross
      console.log(`  ${icon} ${check.name}${!check.passed ? color.dim(` (${check.message})`) : ""}`)
    }
    console.log(color.dim(`  Health: ${doctorResult.passed}/${doctorResult.total} checks passed`))
    console.log()
  }
  ...
  if (!config.hasClaude) {
    ...
    console.log()
    console.log(color.red(color.bold("  Kord agent is STRONGLY optimized for Claude Opus 4.5.")))
  ...
  console.log(`  ${color.dim("gh api --silent --method PUT /user/starred/kord-aios/kord-aios >/dev/null 2>&1 || true")}`)
  console.log()
  console.log(color.dim("Kord AIOS... Enjoy!"))
  console.log()
  ...
  printInfo(`Install method: ${color.cyan("npx kord-aios install")} or ${color.cyan("bunx kord-aios install")}`)
...
export async function install(args: InstallArgs): Promise<number> {
  ...
  p.intro(color.bgMagenta(color.white(isUpdate ? " Kord AIOS Update " : " Kord AIOS Install ")))
  p.log.info(getMaturityMessage(effectiveStatus))
  ...
  s.start("Writing Kord AIOS configuration")
  const kordAiosResult = writeKordAiosConfig(config)
  if (!kordAiosResult.success) {
    s.stop(`Failed to write config: ${kordAiosResult.error}`)
    p.outro(color.red("Installation failed."))
    return 1
  }
  ...
  if (!args.skipDoctor) {
    ...
    const doctorResult = runPostInstallDoctor(process.cwd())
    const doctorLines = doctorResult.checks.map(c => {
      const icon = c.passed ? color.green("✓") : color.red("✗")
      return `${icon} ${c.name}${!c.passed ? color.dim(` (${c.message})`) : ""}`
    })
    doctorLines.push(color.dim(`Health: ${doctorResult.passed}/${doctorResult.total} checks passed`))
    s.stop("Post-install verification complete")
    p.note(doctorLines.join("\n"), "Verification")
  }
  ...
  p.log.message(`  ${color.dim("gh api --silent --method PUT /user/starred/kord-aios/kord-aios >/dev/null 2>&1 || true")}`)

  p.outro(color.green("Kord AIOS... Enjoy!"))
