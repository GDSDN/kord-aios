...
function resolveSelectedCategories(options: ExtractOptions): SelectedCategories {
  const hasCategoryFlag = options.agentsOnly || options.skillsOnly || options.squadsOnly
...
async function collectSkillItems(targetDir: string): Promise<ExtractPlanItem[]> {
  ...
  for (const sourcePath of allFiles) {
    ...
    const relativePath = relative(BUILTIN_SKILLS_DIR, dirname(sourcePath)).replace(/\\/g, "/")
    const destinationPath = join(targetDir, "skills", "kord-aios", relativePath, "SKILL.md")
    items.push({
      category: "skills",
      sourcePath,
      destinationPath,
...
function printSummary(
  ...
  console.log("")
  console.log(isDiff ? "Content sync dry-run summary:" : "Content sync summary:")
