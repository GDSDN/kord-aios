...
export type OpenCodeAvailabilityMethod = "binary" | "bunx" | "none";

export interface OpenCodeAvailability {
  method: OpenCodeAvailabilityMethod;
  installed: boolean;
  available: boolean;
  version: string | null;
  command: string | null;
...
export async function detectOpenCodeAvailability(): Promise<OpenCodeAvailability> {
  const binaryResult = await findOpenCodeBinaryWithVersion();
  if (binaryResult) {
    return {
      method: "binary",
      installed: true,
...
export async function isOpenCodeInstalled(): Promise<boolean> {
  const result = await detectOpenCodeAvailability();
