...
import { LOOK_AT_DESCRIPTION, VISION_AGENT } from "./constants"
import type { LookAtArgs } from "./types"
import { log, promptWithRetry } from "../../shared"
...
Original error: ${createResult.error}`
      ...
      try {
        ...
        if (agents?.length) {
          const matchedAgent = agents.find(
            (agent) => agent.name.toLowerCase() === VISION_AGENT.toLowerCase()
          )
        ...
        log("[look_at] Failed to resolve vision model info", error)
      }
      ...
      try {
        ...
        if (isJsonParseError) {
          return `Error: Failed to analyze ${isBase64Input ? "image" : "file"} - received malformed response from vision agent.
