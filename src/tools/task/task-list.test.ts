...
describe("createTaskList", () => {
  ...
  beforeEach(() => {
    taskDir = join(testProjectDir, "docs", "kord", "tasks")
  ...
  it("returns empty array when no tasks exist", async () => {
    ...
    const config = {
      kord: {
        tasks: {
          storage_path: join(testProjectDir, "docs", "kord", "tasks"),
  ...
  it("excludes completed tasks by default", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-1.json"), task1)
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-2.json"), task2)
  ...
  it("excludes deleted tasks by default", async () => {
   ...
   it("returns summary format with id, subject, status, owner, blockedBy", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-1.json"), task)
    ...
    expect(summary.owner).toBe("kord")
  ...
  it("filters blockedBy to only include unresolved (non-completed) blockers", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-blocker-completed.json"), blockerCompleted)
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-blocker-pending.json"), blockerPending)
    writeJsonAtomic(join(testProjectDir, "docs", "kord", "tasks", "T-main.json"), mainTask)
