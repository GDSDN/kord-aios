...
describe("skill tool - agent restriction", () => {
  ...
  it("allows skill when agent matches restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("restricted-skill", { agent: "kord" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "kord" }
  ...
  it("throws error when agent does not match restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("kord-only-skill", { agent: "kord" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "architect" }

    // when / #then
    await expect(tool.execute({ name: "kord-only-skill" }, context)).rejects.toThrow(
      'Skill "kord-only-skill" is restricted to agent "kord"'
    )
  })
  ...
  it("throws error when context agent is undefined for restricted skill", async () => {
    ...
    // when / #then
    await expect(tool.execute({ name: "kord-only-skill" }, contextWithoutAgent)).rejects.toThrow(
      'Skill "kord-only-skill" is restricted to agent "kord"'
    )
  })
