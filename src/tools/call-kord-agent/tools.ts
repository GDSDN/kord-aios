...
export function createCallKordAgent(
  ...
  const description = CALL_KORD_AGENT_DESCRIPTION.replace("{agents}", agentDescriptions)
  ...
  return tool({
    ...
    async execute(args: CallKordAgentArgs, toolContext) {
      const toolCtx = toolContext as ToolContextWithMetadata
      log(`[call_kord_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`)
...
async function executeBackground(
  ...
  try {
    ...
    log("[call_kord_agent] parentAgent resolution", {
...
async function executeSync(
  ...
  if (args.session_id) {
    log(`[call_kord_agent] Using existing session: ${args.session_id}`)
    const sessionResult = await ctx.client.session.get({
      path: { id: args.session_id },
    })
    if (sessionResult.error) {
      log(`[call_kord_agent] Session get error:`, sessionResult.error)
      return `Error: Failed to get existing session: ${sessionResult.error}`
    }
    sessionID = args.session_id
  } else {
    log(`[call_kord_agent] Creating new session with parent: ${toolContext.sessionID}`)
    const parentSession = await ctx.client.session.get({
      path: { id: toolContext.sessionID },
    }).catch((err) => {
      log(`[call_kord_agent] Failed to get parent session:`, err)
      return null
    })
...
Original error: ${createResult.error}`
  ...
  log(`[call_kord_agent] Sending prompt to session ${sessionID}`)
  ...
  log(`[call_kord_agent] Prompt sent, polling for completion...`)
  ...
  while (Date.now() - pollStart < MAX_POLL_TIME_MS) {
    // Check if aborted
    if (toolContext.abort?.aborted) {
      log(`[call_kord_agent] Aborted by user`)
      return `Task aborted.\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
    }
    ...
    if (currentMsgCount > 0 && currentMsgCount === lastMsgCount) {
      ...
      if (stablePolls >= STABILITY_REQUIRED) {
        log(`[call_kord_agent] Session complete, ${currentMsgCount} messages`)
        break
      }
    } else {
      stablePolls = 0
  ...
  if (Date.now() - pollStart >= MAX_POLL_TIME_MS) {
    log(`[call_kord_agent] Timeout reached`)
  ...
  if (messagesResult.error) {
    log(`[call_kord_agent] Messages error:`, messagesResult.error)
    return `Error: Failed to get messages: ${messagesResult.error}`
  }

  const messages = messagesResult.data
  log(`[call_kord_agent] Got ${messages.length} messages`)
  ...
  if (relevantMessages.length === 0) {
    log(`[call_kord_agent] No assistant or tool messages found`)
    log(`[call_kord_agent] All messages:`, JSON.stringify(messages, null, 2))
    return `Error: No assistant or tool response found\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
  }

  log(`[call_kord_agent] Found ${relevantMessages.length} relevant messages`)
  ...
  log(`[call_kord_agent] Got response, length: ${responseText.length}`)
