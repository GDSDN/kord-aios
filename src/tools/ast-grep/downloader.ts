...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[kord-aios] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[kord-aios] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[kord-aios] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[kord-aios] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
