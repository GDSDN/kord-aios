...
describe("sisyphus-task", () => {
  ...
  describe("category delegation config validation", () => {
    test(
      "fills subagent_type as dev when category is provided without subagent_type",
      async () => {
        // given
        ...
        // then
        expect(args.subagent_type).toBe("dev");
      },
      { timeout: 10000 },
    );
    ...
    test(
      "normalizes legacy subagent_type sisyphus-junior to dev",
      async () => {
        // given
        const { createDelegateTask } = require("./tools");
        let promptBody: any;
        ...
        const mockClient = {
          app: {
            agents: async () => ({ data: [{ name: "dev", mode: "subagent" }] }),
          },
          config: {
          ...
          session: {
            ...
            create: async () => ({ data: { id: "ses_legacy_dev" } }),
            prompt: promptMock,
        ...
        await tool.execute(
          {
            description: "Legacy alias test",
            prompt: "Run with legacy alias",
        ...
        // then
        expect(promptBody.agent).toBe("dev");
      },
      { timeout: 20000 },
    );
  ...
  describe("category variant", () => {
    ...
    test("DEFAULT_CATEGORIES variant passes to background WITHOUT userCategories", async () => {
      ...
      const mockClient = {
        ...
        model: {
          list: async () => [{ provider: "anthropic", id: "claude-opus-4-6" }],
        },
        session: {
          create: async () => ({ data: { id: "test-session" } }),
          prompt: async () => ({ data: {} }),
    ...
    test(
      ...
      async () => {
        ...
        const mockClient = {
          ...
          model: {
            list: async () => [{ provider: "google", id: "gemini-3-pro" }],
          },
          session: {
            get: async () => ({ data: { directory: "/project" } }),
    ...
    test(
      ...
      async () => {
        ...
        const mockClient = {
          ...
          model: {
            list: async () => [{ provider: "google", id: "gemini-3-flash" }],
          },
          session: {
            get: async () => ({ data: { directory: "/project" } }),
  ...
  describe("category model resolution fallback", () => {
    ...
    test("dev model override takes precedence over category model", async () => {
      // given - dev override model differs from category default
      const { createDelegateTask } = require("./tools");
      let launchInput: any;
    ...
    test("explicit category model takes precedence over dev model", async () => {
      // given - explicit category model differs from dev override
      const { createDelegateTask } = require("./tools");
      let launchInput: any;
  ...
  describe("prometheus self-delegation block", () => {
    test("prometheus cannot delegate to prometheus - returns error with guidance", async () => {
      ...
      const mockClient = {
        app: {
          agents: async () => ({
            data: [{ name: "prometheus", mode: "subagent" }],
          }),
        },
        config: {
          get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }),
    ...
    test(
      "non-prometheus agent CAN delegate to prometheus - proceeds normally",
      async () => {
        ...
        const mockClient = {
          ...
          session: {
            ...
            status: async () => ({
              data: { ses_prometheus_allowed: { type: "idle" } },
            }),
          },
        };
  ...
  describe("prometheus subagent task permission", () => {
    test(
      "prometheus subagent should have task permission enabled",
      async () => {
        ...
        const mockClient = {
          ...
          session: {
            ...
            status: async () => ({
              data: { ses_prometheus_delegate: { type: "idle" } },
            }),
          },
        };
    ...
    test(
      "non-prometheus subagent should NOT have task permission",
      async () => {
  ...
  describe("session title and metadata format (OpenCode compatibility)", () => {
    test(
      ...
      async () => {
        ...
        expect(createBody.title).toBe("Implement feature X (@dev subagent)");
      },
      { timeout: 10000 },
    );
    ...
    test(
      ...
      async () => {
        ...
        const tool = createDelegateTask({
          ...
          client: mockClient,
          userCategories: {
            dev: { model: "anthropic/claude-sonnet-4-5" },
          },
        });
