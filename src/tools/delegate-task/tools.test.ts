...
describe("kord-task", () => {
  ...
  describe("category delegation config validation", () => {
    test("fills subagent_type as dev-junior when category is provided without subagent_type", async () => {
      ...
      // then
      expect(args.subagent_type).toBe("dev-junior")
    }, { timeout: 10000 })

    test("executor overrides category and story_path is appended to prompt", async () => {
      //#given
      const { createDelegateTask } = require("./tools")

      const launchCalls: Array<{ agent?: string; prompt?: string }> = []
      const mockManager = {
      ...
      const args: DelegateTaskArgs = {
        ...
        category: "deep",
        executor: "architect",
        story_path: "docs/epics/epic-02/Story.md",
        run_in_background: true,
        load_skills: [],
      }
      ...
      expect(args.subagent_type).toBe("architect")
      expect(launchCalls[0].agent).toBe("architect")
      expect(launchCalls[0].prompt).toContain("Story Path: docs/epics/epic-02/Story.md")
    }, { timeout: 10000 })
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const mockManager = { launch: async () => ({ id: "task-123", status: "pending", description: "Test task", agent: "dev-junior", sessionID: "test-session" }) }
      const mockClient = {
        app: { agents: async () => ({ data: [] }) },
      ...
      expect(result).not.toContain("Kord AIOS requires a default model")
    }, { timeout: 10000 })
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO kordJuniorModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        // userCategories: undefined - use DEFAULT_CATEGORIES only
        // kordJuniorModel: undefined
      })
    ...
    test("dev-junior model override takes precedence over category model", async () => {
      // given - dev-junior override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        kordJuniorModel: "anthropic/claude-sonnet-4-5",
      })
    ...
    test("explicit category model takes precedence over dev-junior model", async () => {
      // given - explicit category model differs from dev-junior override
      const { createDelegateTask } = require("./tools")
      let launchInput: any
  ...
  describe("buildSystemContent", () => {
    ...
    test("prepends plan agent system prompt when agentName is 'Plan' (case insensitive)", () => {
      // given
      const { buildSystemContent } = require("./tools")
      const { buildPlanAgentSystemPrepend } = require("./constants")
  ...
  describe("plan self-delegation block", () => {
    test("plan cannot delegate to plan - returns error with guidance", async () => {
      ...
      // when - plan tries to delegate to plan
      const result = await tool.execute(
        {
      ...
      // then - should return error telling plan to create plan directly
      expect(result).toContain("plan")
      expect(result).toContain("directly")
    })

    test("non-plan agent CAN delegate to plan - proceeds normally", async () => {
      // given - current agent is kord
      const { createDelegateTask } = require("./tools")
      ...
      const mockClient = {
        ...
        session: {
          get: async () => ({ data: { directory: "/project" } }),
          create: async () => ({ data: { id: "ses_plan_allowed" } }),
          prompt: async () => ({ data: {} }),
          messages: async () => ({
            data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Plan created successfully" }] }]
          }),
          status: async () => ({ data: { "ses_plan_allowed": { type: "idle" } } }),
        },
      }
      ...
      // when - kord delegates to plan
      const result = await tool.execute(
        {
          description: "Test plan delegation from non-plan agent",
          prompt: "Create a plan",
          subagent_type: "plan",
          run_in_background: false,
          load_skills: [],
    ...
    test("case-insensitive: Plan (capitalized) cannot delegate to plan", async () => {
      // given - current agent is Plan (capitalized)
      const { createDelegateTask } = require("./tools")
    ...
    test("sync mode passes matched agent model to session.prompt", async () => {
      ...
      const mockClient = {
        app: {
          agents: async () => ({
            data: [
              { name: "architect", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-opus-4-6" } },
            ],
          }),
        },
        config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
        session: {
          ...
          status: async () => ({ data: { "ses_architect_model": { type: "idle" } } }),
        },
      }
  ...
  describe("plan subagent task permission", () => {
    ...
    test("non-plan subagent should NOT have task permission", async () => {
      ...
      // when - kord delegates to architect
      await tool.execute(
        {
  ...
  describe("session title and metadata format (OpenCode compatibility)", () => {
    test("sync session title follows OpenCode format: '{description} (@{agent} subagent)'", async () => {
      ...
      expect(createBody.title).toBe("Implement feature X (@dev-junior subagent)")
    }, { timeout: 10000 })
