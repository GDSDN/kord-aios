...
- category: Use predefined category → Spawns Dev-Junior with category config
  Available categories:
${categoryList}
- subagent_type: Use specific agent directly (e.g., "architect", "explore")
- executor: Override routing with a specific agent (same as subagent_type, but preferred for plan execution)
- story_path: Optional path to story file to include in prompt context
...
Prompts MUST be in English.`
  ...
  return tool({
    ...
    args: {
      ...
      subagent_type: tool.schema.string().optional().describe("Agent name (e.g., 'architect', 'explore'). Mutually exclusive with category."),
      executor: tool.schema.string().optional().describe("Specific agent to execute the task (overrides category routing)."),
      story_path: tool.schema.string().optional().describe("Path to story file for progress tracking."),
    ...
    async execute(args: DelegateTaskArgs, toolContext) {
      ...
      if (args.category && args.subagent_type && args.subagent_type !== "dev-junior") {
