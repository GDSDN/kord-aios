...
Create unified installer that fuses OMOC's interactive setup with AIOS project bootstrap, enabling seamless initialization of Kord AIOS projects.

**Success Criteria:**

- [ ] `kord-aios init [project-name]` creates complete project structure
...
- [ ] Backward compatibility: `kord-aios install` still works
...
- `kord-aios install` - Provider auth, model fallback prompts
...
Unified `kord-aios init` command that:
...
| AC-7  | Post-init validation passes             | `kord-aios doctor` returns exit code 0                                 |
| AC-8  | Init report generated                   | `cat .kord-aios/init-report.json` exists                               |
| AC-9  | Legacy command still works              | `kord-aios install` executes without error                        |
...
- [ ] **2.1.1** Create `bin/kord-aios.js` entry point
  - **AC:** File exists and is executable (`chmod +x`)
  - **AC:** Supports `kord-aios [command] [options]` syntax
  - **AC:** `--version` flag works and shows version
  - **AC:** `--help` flag shows usage information
  - **Verification:** `./bin/kord-aios.js --version` outputs version

- [ ] **2.1.2** Add `init` subcommand
  - **AC:** `kord-aios init --help` shows init-specific help
  ...
  - **Verification:** `kord-aios init --help` shows options

- [ ] **2.1.3** Preserve `kord-aios` compatibility
  - **AC:** `bin/kord-aios.js` still works as entry point
  - **AC:** `kord-aios install` executes install flow
  - **AC:** No breaking changes to existing commands
  - **Verification:** `kord-aios --version` works
...
- [ ] **2.2.2** Create detection report
  ...
  - **Verification:** `kord-aios detect` command shows report
...
- [ ] **2.2.3** Interactive mode for ambiguous cases
  ...
  - **Verification:** Test with `echo "2" | kord-aios init .`
...
- [ ] **2.3.6** Create README
  ...
  - **AC:** Includes Kord AIOS badge and description
...
- [ ] **2.4.3** Store auth config
  ...
  - **Verification:** `cat ~/.config/kord-aios/config.json` shows keys (masked test)
...
- [ ] **2.4.4** Support non-interactive auth
  ...
  - **Verification:** `kord-aios init . --provider anthropic --api-key sk-xxx`
...
- [ ] **2.6.1** Create `kord-aios install-skills` command
  ...
  - **Verification:** `kord-aios install-skills --help`
...
- [ ] **2.6.2** Implement skill discovery
  ...
  - **Verification:** `kord-aios skill list` shows all 176+ skills
...
- [ ] **2.7.1** Create `kord-aios doctor` command
  ...
  - **Verification:** `kord-aios doctor --help`
...
- [ ] **2.7.2** Implement structure validation
  ...
  - **Verification:** `kord-aios doctor` on valid project
...
- [ ] **2.7.3** Implement skill validation
  ...
  - **Verification:** `kord-aios doctor | grep "skills:"`
...
- [ ] **2.7.4** Implement agent validation
  ...
  - **Verification:** `kord-aios doctor | grep "agents:"`

- [ ] **2.7.5** Generate validation report
  - **AC:** Report saved to `.kord-aios/doctor-report.json`
  - **AC:** Includes: checks performed, results, recommendations
  - **AC:** Human-readable summary printed
  - **Verification:** `cat .kord-aios/doctor-report.json | jq .`
...
| ----------------------------- | --------------------------- | ------- |
| `bin/kord-aios.js`            | New CLI entry               | 2.1.1   |
...
| File                    | Reason                     |
| ----------------------- | -------------------------- |
| `bin/kord-aios.js` | Backward compatibility     |
...
# Check report
cat .kord-aios/merge-report.json | jq .
...
# List skills
kord-aios skill list

# Verify count
kord-aios skill list | wc -l  # Should be 176+
```
...
- [ ] Legacy `kord-aios install` still works
...
kord-aios init /tmp/final-test/my-project
kord-aios doctor /tmp/final-test/my-project

# Test merge
cp -r /tmp/final-test/my-project /tmp/final-test/merge-test
kord-aios init /tmp/final-test/merge-test
...
- [Kord AIOS Architecture](../architecture/kord-aios-architecture.md)
