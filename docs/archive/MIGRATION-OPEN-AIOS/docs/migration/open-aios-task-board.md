# Kord AIOS Migration Task Board
...
Kord AIOS fusion combines OMOC engine (40+ hooks, Bun runtime) with AIOS methodology (176+ skills, story-driven workflows). This task board organizes work into **autonomous execution waves**—each wave is self-contained with clear entry/exit criteria and verification commands.
...
- [x] Naming map created (OMOC → Kord AIOS)
...
- `docs/migration/kord-aios-bootstrap-report.json`
...
- Port 9 OMOC agents to canonical Kord AIOS names
...
- [ ] `kord-aios install-skills` command works
...
- [ ] `kord-aios init [project-name]` creates project structure
...
cd existing-project
kord-aios init .  # Should detect existing and merge
test -f .opencode.backup.*/opencode.json  # Backup created

# Verify skills available after init
kord-aios skill list | head -20

# Legacy command still works
kord-aios install --help
```
...
- [ ] `package.json` updated for @synkra/kord-aios
...
ls -la dist/kord-aios-*

# Test binary
./dist/kord-aios --version

# Pack for npm
bun pack

# Verify package contents
tar -tzf synkra-kord-aios-*.tgz | head -30
```
