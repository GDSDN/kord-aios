# Open-AIOS Migration Plan (Detailed)
...
| Phase      | Open-AIOS       | AIOS Ref   | OMOC Alias  | Purpose                     |
| ---------- | --------------- | ---------- | ----------- | --------------------------- |
...
Port AIOS agents to Open-AIOS format while maintaining backward compatibility and infusing methodology.
...
Phase 2: Installation
  - open-aios install-skills command
...
- Preserve existing `oh-my-opencode install` flow
...
Phase 1: CLI Structure
  - bin/open-aios.js entry point
  - init subcommand
  - Preserve oh-my-opencode compatibility
...
cd my-project
open-aios skill list  # Should show 176+ skills
...
| OMOC (Legacy)   | AIOS (Framework) | Open-AIOS (Canonical) |
| --------------- | ---------------- | --------------------- |
...
| from-omoc.md             | Migration from oh-my-opencode              |
...
```json
{
  "name": "@synkra/open-aios",
  "version": "1.0.0",
  "bin": {
    "open-aios": "./bin/open-aios.js",
...
```bash
# Windows
bun build --compile --target=bun-windows-x64 ./src/index.ts --outfile ./dist/open-aios.exe

# macOS
bun build --compile --target=bun-darwin-arm64 ./src/index.ts --outfile ./dist/open-aios-macos
bun build --compile --target=bun-darwin-x64 ./src/index.ts --outfile ./dist/open-aios-macos-intel

# Linux
bun build --compile --target=bun-linux-x64 ./src/index.ts --outfile ./dist/open-aios-linux
```
...
| Method   | Command                                               |
| -------- | ----------------------------------------------------- |
| NPM      | `npm install -g @synkra/open-aios`                    |
| Homebrew | `brew install synkra/tap/open-aios`                   |
| Direct   | `curl -fsSL https://open-aios.dev/install.sh \| bash` |
...
| Feature          | OMOC Status | Open-AIOS Status | Notes                 |
| ---------------- | ----------- | ---------------- | --------------------- |
...
**Objective:** Ensure MCP servers work identically in Open-AIOS as in OMOC.
...
# Verify playwright
open-aios mcp playwright --help

# Verify docker-gateway
open-aios mcp desktop-commander --help
```
...
- Project: **Open-AIOS** (distinct from OpenCode, OMOC)
...
- **oh-my-opencode (OMOC)**: Engine foundation
```
...
Open-AIOS migration complete when:
...
4. ✅ **Installer:** `open-aios init` works on fresh/existing projects
...
- [open-aios-architecture.md](../architecture/open-aios-architecture.md) - Full architecture
...
- [open-aios-task-board.md](./open-aios-task-board.md) - Execution waves
