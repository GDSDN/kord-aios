...
Kord AIOS's orchestration system transforms a simple AI agent into a coordinated development team. This document explains how the Plan → Build → Junior workflow creates high-quality, reliable code output.
...
```mermaid
flowchart TB
    subgraph Planning["Planning Layer (Human + Plan)"]
        User[("👤 User")]
        Plan_Agent["🔥 Plan<br/>(Planner)<br/>Claude Opus 4.5"]
        Analyst["🦉 Analyst<br/>(Consultant)<br/>Claude Opus 4.5"]
        QA["👁️ QA<br/>(Reviewer)<br/>GPT-5.2"]
    end
    
    subgraph Execution["Execution Layer (Orchestrator)"]
        Orchestrator["⚡ Build<br/>(Conductor)<br/>Claude Opus 4.5"]
    end
    
    subgraph Workers["Worker Layer (Specialized Agents)"]
        Junior["🪨 Dev-Junior<br/>(Task Executor)<br/>Claude Sonnet 4.5"]
        Architect["🧠 Architect<br/>(Architecture)<br/>GPT-5.2"]
    ...
    User -->|"Describe work"| Plan_Agent
    Plan_Agent -->|"Consult"| Analyst
    Plan_Agent -->|"Interview"| User
    Plan_Agent -->|"Generate plan"| PlanFile["docs/kord/plans/*.md"]
    PlanFile -->|"High accuracy?"| QA
    QA -->|"OKAY / REJECT"| Plan_Agent
    
    User -->|"/start-work"| Orchestrator
    PlanFile -->|"Read"| Orchestrator
...
## Layer 1: Planning (Plan + Analyst + QA)

### Plan: Your Strategic Consultant

Plan is **not just a planner** - it's an intelligent interviewer that helps you think through what you actually need.
...
Plan adapts its interview style based on what you're doing:

| Intent | Plan Focus | Example Questions |
|--------|------------------|-------------------|
...
Before Plan writes the plan, **Analyst catches what Plan missed**:
...
The plan author (Plan) has "ADHD working memory" - it makes connections that never make it onto the page. Analyst forces externalization of implicit knowledge.

### QA: The Ruthless Reviewer

For high-accuracy mode, QA validates plans against **four core criteria**:
...
If REJECTED, Plan fixes issues and resubmits. **No maximum retry limit.**

---

## Layer 2: Execution (Build)

### The Conductor Mindset

Build is like an orchestra conductor: **it doesn't play instruments, it ensures perfect harmony**.

```mermaid
flowchart LR
    subgraph Orchestrator["Build"]
        Read["1. Read Plan"]
...
This "boulder pushing" mechanism keeps agents persistently working until completion.
...
sequenceDiagram
    participant User
    participant Orchestrator as Build
    participant Junior as Dev-Junior
    participant Notepad as docs/kord/notepads/
...
- **Planning** (Plan): High reasoning, interview, strategic thinking
- **Orchestration** (Build): Coordination, verification, wisdom accumulation
...
Build **never trusts subagent claims**:
...
1. **Enter Plan Mode**: Press **Tab** at the prompt
2. **Describe Your Work**: "I want to add user authentication to my app"
3. **Answer Interview Questions**: Plan will ask about patterns, preferences, constraints
4. **Review the Plan**: Check `docs/kord/plans/` for generated work plan
