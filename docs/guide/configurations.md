...
```jsonc
{
  ...
  "agents": {
    "architect": { "model": "openai/gpt-5.2" },        // Use GPT for debugging
...
The `kord-aios` configuration file supports JSONC (JSON with Comments):
- Line comments: `// comment`
- Block comments: `/* comment */`
- Trailing commas: `{ "key": "value", }`

When both `kord-aios.jsonc` and `kord-aios.json` files exist, `.jsonc` takes priority.
...
Common Ollama models that work with kord-aios:
...
**Tracking**: https://github.com/GDSDN/kord-aios/issues/1124
...
Each agent supports: `model`, `temperature`, `top_p`, `prompt`, `prompt_append`, `tools`, `disable`, `description`, `mode`, `color`, `permission`, `category`, `variant`, `maxTokens`, `thinking`, `reasoningEffort`, `textVerbosity`, `providerOptions`, `routing_mode`, `fallback_slots`.
...
#### Provider-Specific Reasoning Controls (No Abstraction Layer)

Kord AIOS uses provider-specific controls directly: provider fields are passed to the OpenCode SDK as-is. There is **no automatic translation** between a unified "reasoning level" and provider knobs.

**Implication**: if you set an unsupported or invalid value, the provider/SDK may ignore it or return an error. We do not coerce values (e.g., "thinking: low" is not mapped to `enabled`).

| Provider | Field | Expected Values | Notes |
|----------|-------|-----------------|------|
| **Anthropic** | `thinking` | `{ type: "enabled" | "disabled", budgetTokens?: number }` | Extended thinking toggle + optional budget. No "low/medium/high" mapping. |
| **OpenAI** | `reasoningEffort` | `low` \| `medium` \| `high` \| `xhigh` | Passed as-is to OpenAI reasoning controls. |
| **OpenAI** | `textVerbosity` | `low` \| `medium` \| `high` | Passed as-is to OpenAI verbosity controls. |
| **Gemini** | `variant` | `max` \| `high` \| `medium` \| `low` \| `xhigh` | Variant is provider-specific; availability depends on the configured Gemini provider. |
| **Any** | `providerOptions` | object | Raw provider options passed directly to the OpenCode SDK. |

If you need a unified abstraction, we can add a mapping layer (e.g., `reasoning_level`) later. For now, **use provider-specific fields** to avoid surprises.
...
You can also override settings for `kord` (the main orchestrator) and `build` (the default agent) using the same options.
...
Or disable via `disabled_agents` in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:

```json
{
  "disabled_agents": ["architect", "vision"]
}
```
...
- **static** (default): Uses the hardcoded fallback chain. First available model in the chain is used.
- **dynamic**: Uses a heuristic (or optional LLM nano) classifier to analyze task complexity and select the best model from the Model Schema.
...
| Option | Type | Description |
|--------|------|-------------|
| `routing_mode` | `"static"` \| `"dynamic"` | Model selection strategy. Default: `"static"` |
| `fallback_slots` | `string[]` | Custom fallback chain (max 4 entries, format: `"provider/model"`) |

**Prompt-based override**: In any mode, you can override the model directly in your prompt:
- `use model claude-opus-4-6`
- `use openai/gpt-5.2`
- `with model gemini-3-pro`
- `@architect must use claude-opus-4-6`
...
{
  "model_routing": {
    "classifier": "heuristic",    // "heuristic" (default, zero latency) or "llm" (uses nano model)
    "cost_preference": "balanced", // "economy", "balanced", or "performance"
    "prompt_override": true        // Allow prompt-based model override (default: true)
  }
}
```
...
The Model Schema classifies models by reasoning tier, domains, and cost for dynamic routing. It ships with sensible defaults and can be customized:

- **User-level**: `~/.config/opencode/kord-aios/model-schema.jsonc`
- **Project-level**: `.opencode/kord-aios/model-schema.jsonc`

Project overrides user, user overrides defaults. Entries are matched by `model` name.
  ...
  "models": [
    {
      ...
      "reasoning": "ultra",           // none | low | medium | high | ultra
      "domains": ["planning", "coding"], // planning | coding | analysis | visual | writing | search | general
      "description": "Deep reasoning",
      "enabled_agents": ["kord", "plan"],
      "cost_tier": 5,                 // 1-5 (1=cheapest)
      "variant": "max"                // optional
    }
  ]
}
```
...
Use `/modelconfig` in OpenCode to interactively view and configure model routing:

```
/modelconfig           — Show all agents with current routing
/modelconfig kord      — Configure kord agent routing
```

The TUI shows agent routing modes, fallback slots, connected providers, available models, and the Model Schema. Changes are persisted to `kord-aios.jsonc`.

## Built-in Skills

Kord AIOS includes built-in skills that provide additional capabilities:
...
Disable built-in skills via `disabled_skills` in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:
...
| `compatibility`    | -       | Required kord-aios version compatibility                                           |
...
**Switch providers** via `browser_automation_engine` in `kord-aios.json`:
...
**Enable tmux integration** via `tmux` in `kord-aios.json`:
...
2. When a background agent spawns, Kord AIOS creates a new tmux pane
...
| Option                   | Default | Description                                                                      |
| ------------------------ | ------- | -------------------------------------------------------------------------------- |
| `commit_footer`          | `true`  | Adds "Ultraworked with Kord" footer to commit messages.                      |
| `include_co_authored_by` | `true`  | Adds `Co-authored-by: Kord <kord-aios@users.noreply.github.com>` trailer to commits. |

## Kord Agent

When enabled (default), Kord provides a powerful orchestrator with optional specialized agents:

- **Kord**: Primary orchestrator agent (Claude Opus 4.5)
- **OpenCode-Builder**: OpenCode's default build agent, renamed due to SDK limitations (disabled by default)
- **Plan**: OpenCode's default plan agent with work-planner methodology (enabled by default)
- **Analyst**: Pre-planning analysis agent that identifies hidden requirements and AI failure points
...
This enables OpenCode-Builder agent alongside Kord. The default build agent is always demoted to subagent mode when Kord is enabled.

**Example: Disable all Kord orchestration:**
...
You can also customize Kord agents like other agents:
...
| Option                    | Default | Description                                                                                                                            |
| ------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `disabled`                | `false` | When `true`, disables all Kord orchestration and restores original build/plan as primary.                                          |
| `default_builder_enabled` | `false` | When `true`, enables OpenCode-Builder agent (same as OpenCode build, renamed due to SDK limitations). Disabled by default.             |
| `planner_enabled`         | `true`  | When `true`, enables Plan agent with work-planner methodology. Enabled by default.                                     |
| `replace_plan`            | `true`  | When `true`, demotes default plan agent to subagent mode. Set to `false` to keep both Plan and default plan available. |
...
Categories enable domain-specific task delegation via the `task` tool. Each category applies runtime presets (model, temperature, prompt additions) when calling the `dev-junior` agent.
...
```
1. User-configured model (in kord-aios.json)
...
// kord-aios.json (empty categories section)
{}
...
task(agent="architect", prompt="Review this architecture")
```
...
At runtime, Kord AIOS uses a 3-step resolution process to determine which model to use for each agent and category. This happens dynamically based on your configuration and available models.
...
1. **Step 1: User Override** — If you specify a model in `kord-aios.json`, use exactly that
...
│   ┌─────────────────────────────────────────────────────────┐   │
│   │ User specified model in kord-aios.json?            │   │
...
|-------|-------------------|-------------------------|
| **Kord** | `claude-opus-4-6` | anthropic → kimi-for-coding → zai-coding-plan → openai → google |
| **Architect** | `gpt-5.2` | openai → google → anthropic |
| **Librarian** | `glm-4.7` | zai-coding-plan → opencode → anthropic |
| **Explore** | `claude-haiku-4-5` | anthropic → github-copilot → opencode |
| **Vision** | `gemini-3-flash` | google → openai → zai-coding-plan → kimi-for-coding → anthropic → opencode |
| **Plan** | `claude-opus-4-6` | anthropic → kimi-for-coding → openai → google |
| **Analyst** | `claude-opus-4-6` | anthropic → kimi-for-coding → openai → google |
| **QA** | `gpt-5.2` | openai → anthropic → google |
...
Override any agent or category model in `kord-aios.json`:
...
Disable specific built-in hooks via `disabled_hooks` in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:
...
Disable specific built-in commands via `disabled_commands` in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:
...
Configure Kord Tasks system for advanced task management.
...
Don't want them? Disable via `disabled_mcps` in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:
...
Kord AIOS adds refactoring tools (rename, code actions).
All OpenCode LSP configs and custom settings (from opencode.json) are supported, plus additional Kord AIOS-specific settings.

Add LSP servers via the `lsp` option in `~/.config/opencode/kord-aios.json` or `.opencode/kord-aios.json`:
