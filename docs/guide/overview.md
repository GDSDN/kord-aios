...
Learn about Kord AIOS, a plugin that transforms OpenCode into the best agent harness.

---

## TL;DR

> **Kord agent strongly recommends Opus 4.5 model. Using other models may result in significantly degraded experience.**

**Feeling lazy?** Just include `ultrawork` (or `ulw`) in your prompt. That's it. The agent figures out the rest.

**Need precision?** Press **Tab** to enter Plan mode, create a work plan through an interview process, then run `/start-work` to execute it with full orchestration.

---

## What Kord AIOS Does for You
...
### Option 2: Plan Mode (For Precise Work)

For complex or critical tasks, press **Tab** to switch to Plan mode.

**How it works:**

1. **Plan interviews you** - Acts as your personal consultant, asking clarifying questions while researching your codebase to understand exactly what you need.

2. **Plan generation** - Based on the interview, Plan generates a detailed work plan with tasks, acceptance criteria, and guardrails. Optionally reviewed by QA (plan reviewer) for high-accuracy validation.

3. **Run `/start-work`** - Build takes over:
...
### Always Use Plan + Build Together

**Do NOT use `build` without `/start-work`.**

Build is designed to execute work plans created by Plan. Using it directly without a plan leads to unpredictable behavior.

**Correct workflow:**
```
1. Press Tab → Enter Plan mode
2. Describe work → Plan interviews you
3. Confirm plan → Review docs/kord/plans/*.md
4. Run /start-work → Build executes
```

**Plan and Build are a pair. Always use them together.**

---

## Model Configuration

Kord AIOS automatically configures models based on your available providers. You don't need to manually specify every model.

### How Models Are Determined

**1. At Installation Time (Interactive Installer)**

When you run `bunx kord-aios install`, the installer asks which providers you have:
...
Based on your answers, it generates `~/.config/opencode/kord-aios.json` with optimal model assignments for each agent and category.
...
```jsonc
{
  ...
  "agents": {
    ...
    "build": { "model": "anthropic/claude-sonnet-4-5", "variant": "max" },
...
- [Understanding the Orchestration System](./understanding-orchestration-system.md) - Deep dive into Plan → Build → Junior workflow
- [Ultrawork Manifesto](../ultrawork-manifesto.md) - Philosophy and principles behind Kord AIOS
