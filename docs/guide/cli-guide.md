...
This document provides a comprehensive guide to using the Kord AIOS CLI tools.

## 1. Overview

Kord AIOS provides CLI tools accessible via the `bunx kord-aios` command. The CLI supports various features including plugin installation, environment diagnostics, and session execution.
...
An interactive installation tool for initial Kord AIOS setup. Provides a beautiful TUI (Text User Interface) based on `@clack/prompts`.
...
3. **Configuration File Creation**: Generates `opencode.json` or `kord-aios.json` files.
4. **Plugin Registration**: Automatically registers the kord-aios plugin in OpenCode settings.
...
Diagnoses your environment to ensure Kord AIOS is functioning correctly. Performs 17+ health checks.
...
bunx kord-aios mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes "read,write"

# Remove stored OAuth tokens
bunx kord-aios mcp oauth logout <server-name>

# Check OAuth token status
bunx kord-aios mcp oauth status [server-name]
```
...
```bash
# Login
bunx kord-aios auth login

# Logout
bunx kord-aios auth logout

# Check current status
bunx kord-aios auth status
```
...
bunx kord-aios doctor --no-tui --json

# Save results to file
bunx kord-aios doctor --json > doctor-report.json
```
