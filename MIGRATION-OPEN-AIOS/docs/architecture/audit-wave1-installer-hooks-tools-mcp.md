...
This audit evaluates the current state of the open-aios codebase across five critical areas:
...
The codebase shows a mature OMOC engine with 40+ hooks, 25+ tools, and 176+ skills. However, several gaps exist between the current state and the target AIOS architecture, particularly in QA agent invocation, MCP consistency, and CLI integration.
...
|-----|----------|--------------|
| CLI naming inconsistency | Medium | Package name is `open-aios`, but uses `oh-my-opencode` as alias |
| No AIOS layer sync command | High | Need `open-aios sync` to update skills/rules from layer/aios |
| Missing `doctor` implementation | Medium | `doctor` command is wired but incomplete in CLI index |
| No MCP status/list commands | Medium | Kord CLI has MCP status, not exposed in open-aios CLI |
...
src/cli/install.ts
  - Rename from oh-my-opencode to open-aios branding (line 65)
...
1. **Phase 1 (Safe)**: Fix branding inconsistencies
   - Rename console output from "oh-my-opencode" to "open-aios"
   - Test: `bunx open-aios install --no-tui --claude=yes --gemini=yes`

2. **Phase 2 (Safe)**: Add missing CLI commands
   - Implement `doctor` with existing checks from kord.js
   - Test: `bunx open-aios doctor`

3. **Phase 3 (Medium)**: MCP integration
   - Add `open-aios mcp install/status` commands
   - Test: `bunx open-aios mcp install --mcp=exa,context7`

4. **Phase 4 (Medium)**: AIOS sync command
   - Add `open-aios sync` for layer updates
   - Test: `bunx open-aios sync --layer=skills,rules`
...
**25+ tools** across categories including delegate-task, call_omo_agent, skill, skill_mcp, slashcommand, background_output, session-manager, task CRUD, grep/glob/ast-grep, interactive_bash, look_at, lsp.

#### Key Implementation Details:

**`src/tools/call-omo-agent/tools.ts`** (375 lines):
...
- **NOT callable via call_omo_agent** (not in ALLOWED_AGENTS)
...
| Gap | Severity | Target State |
|-----|----------|--------------|
| QA agent not in ALLOWED_AGENTS | **Critical** | Add "qa" to `call_omo_agent` allowed list |
...
3. **Phase 3 (Medium)**: Add CLI commands
   - Implement `open-aios mcp list/status/verify`
   - Test: `bunx open-aios mcp status`
...
- `oh-my-opencode.json` (legacy) still referenced in some places

**Gaps:**
1. Inconsistent naming between open-aios and oh-my-opencode
...
src/cli/migrate-config.ts (new)
  - Create migration tool from oh-my-opencode.json
```
...
bun run audit:omoc-drift    # OMOC drift detection
...
**Present:**
- `docs/architecture/open-aios-architecture.md` - Good overview
...
5. Migration guide from OMOC to open-aios
...
bun run build
# In OpenCode session:
# call_omo_agent(subagent_type="qa", prompt="Review src/index.ts for bugs")

# Test 2: Branding consistency
grep -r "oh-my-opencode" src/cli/*.ts | grep -v "node_modules"
...
```bash
# Test 1: MCP CLI
bunx open-aios mcp list
bunx open-aios mcp status
bunx open-aios mcp install --mcp=exa,context7 --api-key=EXA_API_KEY=xxx
...
```bash
# Test 1: Sync command
bunx open-aios sync --layer=skills --dry-run
bunx open-aios sync --layer=skills,rules
...
```bash
# Test 1: Docker MCP
bunx open-aios mcp install --mcp=apify --docker
...
package.json - NPM package config (open-aios v3.3.0)
```
...
1. **Add "qa" to ALLOWED_AGENTS** in `src/tools/call-omo-agent/constants.ts`
2. **Fix branding** in CLI output (oh-my-opencode to open-aios)
...
### Differences
| Aspect | open-aios (D:\dev\open-aios) | aios-core (D:\dev\aios-core) |
|--------|------------------------------|------------------------------|
| Primary runtime | Bun/Node.js | OpenCode plugin + Docker |
| Agent invocation | call_omo_agent tool | Native skill execution |
| MCP transport | stdio, SSE | stdio, SSE, Docker gateway |
| QA agent | Defined but not callable | Fully integrated |
| CLI | open-aios CLI | Kord CLI + npx @synkra/aios-core |
| Story workflow | Partial | Full with quality gates |
| Hook system | 43 hooks (OMOC) | 37 hooks (Python/JS mix) |
